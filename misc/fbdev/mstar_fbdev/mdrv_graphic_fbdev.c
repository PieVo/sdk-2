...
#define MIU2_INTERVAL     (ARM_MIU2_BUS_BASE - ARM_MIU0_BUS_BASE)
...
static void _fb_gwin_init(struct fb_info *pinfo)
{
   ...
   //Temp patch for ARGB1555 on I2 platform
   if (par->stGwinInfo.clrType == E_DRV_FB_GOP_COLOR_ARGB1555) {
        mstar_FB_SetBlending(u8GopIdx, u8GwinIdx, FALSE, 0xFF);
        par->stAlpha.u8GlobalAlpha = 0xFF;
        par->stAlpha.bAlphaEnable = FALSE;
   } else {
        ...
        case FBIOSET_GLOBAL_ALPHA:
        {
            ...
            if (!data.alpahInfo.bAlphaEnable) {
                ...
                if (currentFmt == E_DRV_FB_GOP_COLOR_ARGB1555) {
                    mstar_FB_SetAlpha0(gopId, data.alpahInfo.u8Alpha0);
                    mstar_FB_SetAlpha1(gopId, data.alpahInfo.u8Alpha1);
                }
