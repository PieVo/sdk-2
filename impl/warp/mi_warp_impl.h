...
#define TEST_DEV_WORKTHREAD         0   // set 1 to test
...
{
    MI_WARP_InputPortAttr_t stAttr;
    MI_WARP_InputPort_Status_e eStatus;
...
{
    MI_BOOL bInited;
    MI_WARP_OutputPortAttr_t stAttr;
...
typedef struct MI_WARP_Channel_s
{
    ...
    struct semaphore semChnTest;
#else
    struct task_struct *getInBufThread;
    struct task_struct *putOutBufThread;
    struct semaphore semChnInputBuf;
    struct semaphore semChnOutputBuf;
    MI_WARP_BufQueue_t stGetQueue;
    MI_WARP_BufQueue_t stPutQueue;
#endif
    struct mutex mtxWarpConf;           // config buf
    struct semaphore semTriggerConf;    // framedata proc sema
}MI_WARP_Channel_t;
...
typedef struct MI_WARP_Device_s
{
    ...
    struct semaphore chnCtxSem[MI_WARP_MAX_CHN_NUM];
#endif
    MI_U32 u32WakeEvent;
