...
#define MI_RGN_OVERLAY_MAX_WIDTH 1920
#define MI_RGN_OVERLAY_MIN_HEIGHT 2
#define MI_RGN_OVERLAY_MAX_HEIGHT 1080
...
#define MI_RGN_ENABLE_COLOR_KEY 0
#define MI_RGN_INIT_ALPHA_VALUE 0xFF
...
static MI_U32 _mi_rgn_drv_calpixstride(MI_RGN_PixelFormat_e ePixelFmt,  MI_RGN_Size_t *pstSize)
{
    ...
    switch (ePixelFmt)
    {
        case E_MI_RGN_PIXEL_FORMAT_ARGB1555:
        case E_MI_RGN_PIXEL_FORMAT_ARGB4444:
            u32PixStrideByte = ALIGN_UP(pstSize->u32Width, 8) * 2;
            break;
        case E_MI_RGN_PIXEL_FORMAT_I2:
            u32PixStrideByte = ALIGN_UP(pstSize->u32Width, 64) / 4;
            break;
        case E_MI_RGN_PIXEL_FORMAT_I4:
            u32PixStrideByte = ALIGN_UP(pstSize->u32Width, 32) / 2;
            break;
        default:
            return MI_ERR_RGN_ILLEGAL_PARAM;
    }

    return u32PixStrideByte;
}
...
static MI_S32 _mi_rgn_drv_get_capability(mi_rgn_DrvCapability_t *pstCap)
{
    ...
    pstCap->u8OsdHwModeCnt = MI_RGN_MAX_GOP_GWIN_NUM;
...
//1555 4444
    ...
    MI_SYS_BUG_ON(!pstSurface->virtAddr);
    ...
    if (pstRectPos->u32X < pstSurface->stSize.u32Width && pstRectPos->u32Y < pstSurface->stSize.u32Height)
    {
        ...
        else
        {
            switch (pstSurface->ePixelFmt)
            {
                case E_MI_RGN_PIXEL_FORMAT_ARGB1555:
                {
                    u16Color = (u32ColorVal & 0x1F) | ((u32ColorVal & 0x1F00) >> 3) | (u32ColorVal & 0x1F0000) >> 6 | ((u32ColorVal & 0x1000000) >> 9);
                }
                break;
                case E_MI_RGN_PIXEL_FORMAT_ARGB4444:
                {
                    u16Color = (u32ColorVal & 0xF) | ((u32ColorVal & 0xF00) >> 4) | (u32ColorVal & 0xF0000) >> 8 | ((u32ColorVal & 0xF000000) >> 12);
                }
                break;
                default:
                {
...
#else
            memset((MI_U8 *)pstBufferCfg->virtAddr, MI_RGN_INIT_ALPHA_VALUE, u32Size);
#endif
        }
        break;
        default:
...
static MI_S32 _mi_rgn_drv_misys_buf_alloc(MI_RGN_CanvasInfo_t *pstBufferCfg)
{
    ...
    pstBufferCfg->virtAddr = (MI_VIRT)NULL;
    pstBufferCfg->phyAddr = (MI_PHY)NULL;
    ...
    if (NULL == pstBufferCfg->virtAddr)
    {
        DBG_EXIT_ERR("Sys map error!\n");
        s32Error = MI_ERR_RGN_BADADDR;
        goto MAP_ERROR;
    }
    _mi_rgn_drv_fill_all_alpha_data_by_sw(pstBufferCfg);
    mi_rgn_MemAllocCnt((void *)pstBufferCfg->virtAddr, u32Size, 0);
...
static MI_S32 _mi_rgn_drv_misys_buf_del(MI_RGN_CanvasInfo_t *pstBuffer)
{
    ...
    mi_rgn_MemFreeCnt((void *)pstBuffer->virtAddr, 0);
    mi_sys_UnVmap((void *)pstBuffer->virtAddr);
...
static MI_S32 _mi_rgn_drv_misys_buf_dup(MI_RGN_CanvasInfo_t *pstDupBuffer, MI_RGN_CanvasInfo_t *pstSrcBuffer)
{
    ...
    mi_rgn_MemAllocCnt((void *)pstDupBuffer->virtAddr, u32Size, 0);
...
static MI_S32 _mi_rgn_drv_buf_fill_alpha(MI_RGN_FrontBufferInfo_t *pstFrontBuffer,  MI_RGN_Size_t *pstRectSize, MI_RGN_Point_t *pstRectPos)
{
    ...
    switch (pstFrontBuffer->stFront.ePixelFmt)
    {
        ...
        case E_MI_RGN_PIXEL_FORMAT_ARGB1555:
            {
#if (MI_RGN_ENABLE_COLOR_KEY)
                u32Color = ((MI_RGN_COLOR_KEY_VALUE & 0x8000) << 9)
                    | ((MI_RGN_COLOR_KEY_VALUE & 0x7C00) << 6)
                    | ((MI_RGN_COLOR_KEY_VALUE & 0x3E0) << 3)
                    | (MI_RGN_COLOR_KEY_VALUE & 0x1F);
#else
        ...
        case E_MI_RGN_PIXEL_FORMAT_ARGB4444:
            {
                ...
                u32Color = ((MI_RGN_COLOR_KEY_VALUE & 0xF000) << 12)
                    | ((MI_RGN_COLOR_KEY_VALUE & 0xF00) << 8)
                    | ((MI_RGN_COLOR_KEY_VALUE & 0xF0) << 4)
                    | (MI_RGN_COLOR_KEY_VALUE & 0xF);
#else
                u32Color = MI_RGN_INIT_ALPHA_VALUE << 24;
#endif
            }
    ...
    *pbOverLap = FALSE;
    list_for_each_entry(pos, pTmpList, chPortListTmp)
    {
        ...
        *pbOverLap = MI_RGN_CHECK_XPOS_OVERLAP(u32XposLt, u32XposRt,    \
                ...
                pos->stAttachedChnPortPara.stPoint.u32X + pos->stRgnAttr.stOsdInitParam.stSize.u32Width);
    }
...
static MI_S32 _mi_rgn_drv_build_frontbuffer(MI_RGN_ChPorHeadData_t *pstChPortHeadData, MI_RGN_ChPortListData_t *pstChPortListData, struct list_head *pTmpList)
{
    ...
    MI_BOOL bOverLap = FALSE;
    MI_U16 u16Distance = 0xFFFF;
    MI_U16 u16CurDistance = 0xFFFF;
...
static MI_S32 _mi_rgn_drv_caculate_frontbuffer_size(MI_RGN_FrontBufferInfo_t *pstFrontBuffer, MI_RGN_CanvasInfo_t *pstCanvasInfo, MI_RGN_Point_t *pstPos, struct list_head *pTmpList)
{
    ...
    MI_RGN_PTR_CHECK(pstFrontBuffer, MI_SYS_BUG(););
    ...
    pstCanvasInfo->stSize.u32Width = ALIGN_UP(u32MaxWidth - stPos.u32X, 2);
    pstCanvasInfo->ePixelFmt = eFormat;
    pstPos->u32X = stPos.u32X;
...
static MI_S32 _mi_rgn_drv_do_frontbuffer(MI_RGN_ChPorHeadData_t *pstChPortHeadData, MI_RGN_ChPortListData_t *pstChPortListData)
{
    ...
    /*Third prepare to operate buffer.*/
    for (i = 0; i < MI_RGN_MAX_GOP_GWIN_NUM; i++)
    {
        pstFrontBuffer = &pstChPortHeadData->astFrontBufferInfo[i];
        memcpy(&stFrontBk[i], &pstChPortHeadData->astFrontBufferInfo[i].stFront, sizeof(MI_RGN_CanvasInfo_t));
        memcpy(&stPointBk[i], &pstChPortHeadData->astFrontBufferInfo[i].stPoint, sizeof(MI_RGN_Point_t));

        if (list_empty(&stTmpChnPortListHead[i]))
        {
            if (!_mi_rgn_drv_check_canvasinfo_empty(&stFrontBk[i]))
            {
        ...
        {
            memset(&stCanvasInfo, 0, sizeof(MI_RGN_CanvasInfo_t));
            s32Ret = _mi_rgn_drv_caculate_frontbuffer_size(pstFrontBuffer, &stFrontBk[i], &stPointBk[i], &stTmpChnPortListHead[i]);
            if (s32Ret != MI_RGN_OK)
            {
                memset(&stFrontBk[i], 0, sizeof(MI_RGN_CanvasInfo_t));
                goto ERROR_CODE;
...
#else
    _stPaletteTable.astElement[0].u8Alpha = MI_RGN_INIT_ALPHA_VALUE;
...
static MI_BOOL _mi_rgn_drv_hal_config_color(MHAL_RGN_GopType_e eGopId, MI_RGN_PixelFormat_e eFormat)
{
#if (MI_RGN_ENABLE_COLOR_KEY)
    MI_U8 u8R, u8G, u8B;
#endif
    ...
    switch (eFormat)
    {
        ...
        case E_MI_RGN_PIXEL_FORMAT_I4:
        {
            for (i = 0; i < MI_RGN_MAX_PALETTE_TABLE_NUM; i++)
            {
                MHAL_RGN_GopSetPalette(eGopId, _stPaletteTable.astElement[i].u8Alpha, _stPaletteTable.astElement[i].u8Red, _stPaletteTable.astElement[i].u8Green, _stPaletteTable.astElement[i].u8Blue, i);
            }
...
static MI_S32 _mi_rgn_drv_window_update(MI_RGN_ChnPort_t *pstChnPort,  MI_RGN_Size_t *pstDispWindowSize, MI_BOOL bEn, MHAL_CMDQ_CmdqInterface_t* pstCmdInf, MI_U64 *pu64Fence)
{
    ...
    if (bEn)
    {
        ...
        for (i = 0; i < MI_RGN_MAX_GOP_GWIN_NUM; i++)
        {
            ...
            {
                if (bConfigColor == FALSE)
                {
                    //Color config can not set by gwin.
                    bConfigColor = _mi_rgn_drv_hal_config_color(eGwinId, pstChPortHeadData->astFrontBufferInfo[i].stFront.ePixelFmt);
                }
...
MI_U32 mi_rgn_drv_init(void)
{
    ...
    for (i = 0; i < E_MHAL_GOP_TYPE_MAX; i++)
    {
        for (j = 0 ; j < E_MHAL_GOP_GWIN_ID_MAX; j++)
        {
            ...
            MHAL_RGN_GopSetArgb1555AlphaVal(i, j, E_MHAL_GOP_GWIN_ARGB1555_DEFINE_ALPHA1, MI_RGN_INIT_ALPHA_VALUE);
