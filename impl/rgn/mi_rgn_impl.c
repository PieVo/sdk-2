...
static MI_S32 _MI_RGN_IMPL_DumpFrontBufToFile(MI_SYS_DEBUG_HANDLE_t handle, MI_U32 u32DevId, MI_U8 argc, MI_U8 **argv, void *pUsrData)
{
    ...
    for (i = 0; i < _gstCapablity.u8OsdHwModeCnt; i++)
    {
        ...
        if (stFrontBuff.phyAddr == NULL && stFrontBuff.virtAddr == 0 && stFrontBuff.stSize.u32Width == 0 &&
        ...
        _MI_RGN_IMPL_DumpInfoToFile(pFileName, (MI_U8*)stFrontBuff.virtAddr, stFrontBuff.stSize.u32Height*stFrontBuff.u32Stride);
...
static MI_S32 _MI_RGN_IMPL_DumpRgnBufToFile(MI_SYS_DEBUG_HANDLE_t handle, MI_U32 u32DevId, MI_U8 argc, MI_U8 **argv, void *pUsrData)
{
    ...
    if (pstHl)
    {
        ...
        _MI_RGN_IMPL_DumpInfoToFile(pFileName, (MI_U8*)pstHl->stCanvasInfo.virtAddr, pstHl->stCanvasInfo.stSize.u32Height*pstHl->stCanvasInfo.u32Stride);
...
{
    MI_RGN_PaletteTable_t stPalette;
    MI_U8 u8Idx;

    handle.OnPrintOut(handle, "\n------------------------------------- start dump palette--------------------------------------------------------------\n");
    handle.OnPrintOut(handle, "%8s%8s%8s%8s%8s\n", "Index", "Alpha", "Red", "Green", "Blue");
    _stRegionlist.stWinManger.fpGetPalette(&stPalette);
    for (u8Idx = 0; u8Idx < MI_RGN_MAX_PALETTE_TABLE_NUM; u8Idx++)
    {
        handle.OnPrintOut(handle, "%8d%8x%8x%8x%8x\n", u8Idx, stPalette.astElement[u8Idx].u8Alpha, stPalette.astElement[u8Idx].u8Red, stPalette.astElement[u8Idx].u8Green, stPalette.astElement[u8Idx].u8Blue);
    }
    handle.OnPrintOut(handle, "\n------------------------------------- end dump palette--------------------------------------------------------------\n");
...
static MI_S32 _MI_RGN_IMPL_OnDumpDevAttr(MI_SYS_DEBUG_HANDLE_t handle, MI_U32 u32DevId,void *pUsrData)
{
    ...
    _MI_RGN_IMPL_DumpPalette(handle, u32DevId, 0, NULL, pUsrData);
...
MI_S32 MI_RGN_IMPL_Init(MI_RGN_PaletteTable_t *pstPaletteTable)
{
    ...
    if (!bUseInitFlag)
    {
        ...
        _stRegionlist.stWinManger.fpInitPalette(pstPaletteTable);
    ...
    else
    {
        ...
        eModId = (MI_RGN_ModId_e)pstPaletteTable->astElement[15].u8Alpha;
        stProcessCmdInfo.u32chnID = (MI_U32)pstPaletteTable->astElement[15].u8Green;
    ...
    _stRegionlist.bInitFlag = TRUE;
    _stRegionlist.stWinManger.fpSetPalette(pstPaletteTable);
    memset(&_gstCapablity, 0, sizeof(mi_rgn_DrvCapability_t));
    _stRegionlist.stWinManger.fpGetCapability(&_gstCapablity);
#endif
 ...
 MI_S32 MI_RGN_IMPL_SetBitMap(MI_RGN_HANDLE hHandle,  MI_RGN_Bitmap_t *pstBitmap)
{
    ...
    u32Width = MI_RGN_MIN(pstBitmap->stSize.u32Width, pstLd->stCanvasInfo.stSize.u32Width);
    u32Height = MI_RGN_MIN(pstBitmap->stSize.u32Height, pstLd->stCanvasInfo.stSize.u32Height);

    MI_RGN_BITS_PER_PIXEL(u32Bits, u32Width, pstBitmap->ePixelFormat);
    ...
    pAddr = (MI_U8 *)pstLd->stCanvasInfo.virtAddr;

    for (i = 0; i < u32Height; i++)
    {
        copy_from_user(pAddr, (void *)stCanvasInfo.virtAddr, u32CopyByte);
        pAddr += pstLd->stCanvasInfo.u32Stride;
        stCanvasInfo.virtAddr += stCanvasInfo.u32Stride;
    }
