...
void _MI_SYS_IMPL_ModuleIdToPrefixName(MI_ModuleId_e eModuleId , char *prefix_name)
{
    switch(eModuleId)
    {
        ...
        case E_MI_MODULE_ID_WARP:
            strcpy(prefix_name,"mi_wrap");
            break;
        case E_MI_MODULE_ID_CV:
            strcpy(prefix_name,"mi_cv");
            break;
        case E_MI_MODULE_ID_CEVA:
            strcpy(prefix_name,"mi_ceva");
            break;
        default:
...
MI_S32  dump_buf_data(MI_U32 index,struct queue_dump_buf_cmd_info * dump_buf_cmd_info,MI_SYS_BufRef_t *pstBufRef)
{
    ...
    else if(E_MI_SYS_BUFDATA_FRAME == pstBufRef->bufinfo.eBufType)
    {
        ...
        u32BufSize = _mi_sys_calc_buf_size_from_frame_bufinfo(_mi_sys_get_fmt_alignment(pstBufRef->bufinfo.stFrameData.ePixelFormat),pstBufRef->bufinfo.stFrameData);
...
static void dump_Queue_buf_worker(struct work_struct *work)
{
    ...
    if(E_MI_MODULE_DUMP_BUFFER_INPUT_PORT == dump_buf_cmd_info->port_type)
    {
        ...
        {
            ret = Dump_Buffer_by_Queue(dump_buf_cmd_info,&pstModDev->astChannels->pastInputPorts[dump_buf_cmd_info->port_id]->stUsrInjectBufQueue);
        }
        else if(E_MI_MODULE_DUMP_BUFFER_Queue_BindInput == dump_buf_cmd_info->Queue_name)
        {
            DBG_INFO("[debug][%s:%d]\n",__FUNCTION__,__LINE__);
            ret = Dump_Buffer_by_Queue(dump_buf_cmd_info,&pstModDev->astChannels->pastInputPorts[dump_buf_cmd_info->port_id]->stBindInputBufQueue);
            DBG_INFO("[debug][%s:%d]\n",__FUNCTION__,__LINE__);
        }
        else
        {DBG_INFO("[debug][%s:%d]\n",__FUNCTION__,__LINE__);
    ...
    else if(E_MI_MODULE_DUMP_BUFFER_OUTPUT_PORT == dump_buf_cmd_info->port_type)
    {
         ...
         {
             ret = Dump_Buffer_by_Queue(dump_buf_cmd_info,&pstModDev->astChannels->pastOutputPorts[dump_buf_cmd_info->port_id]->stUsrGetFifoBufQueue);
         }
         else
         {
             MI_SYS_BUG();
