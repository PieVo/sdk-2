...
static ssize_t mi_global_info_show(struct seq_file *m,void *v)
{
    ...
    if(0 == riu_base)
    {
        ...
        {
            seq_printf(m,"MDrv_MMIO_GetBASE fail\n");
            return E_MI_ERR_FAILED;
        }
        else
        {
            riu_base = _PMRIUBaseAddress;
        }
    }

    seq_printf(m,"CHIP_VERSION  U0%d\n",1+MDrv_ReadByte(REG_CHIP_REVISION));//chip version is U01 or U02 or U03
