...
MI_S32 _MI_SYS_IMPL_UnMmapUserVirAddr(MI_SYS_BufRef_t *pstBufRef)
{
...
MI_S32 _MI_SYS_IMPL_MmapToUserVirAddr(MI_SYS_BufRef_t *pstBufRef)
{
    MI_U64 u64Offset;
...
MI_S32 MI_SYS_IMPL_MmapBufToUser(MI_SYS_DRV_HANDLE miSysDrvHandle, MI_SYS_BufInfo_t *pstBufInfo, void **ppBufHandle)
{
    ...
    if (MI_SUCCESS != _MI_SYS_IMPL_MmapToUserVirAddr(pstBufRef))
    {
        kfree(phBufIdrData);
        phBufIdrData = NULL;
...
MI_S32 MI_SYS_IMPL_UnmapBufToUser(MI_SYS_DRV_HANDLE miSysDrvHandle, void *pBufHandle, MI_SYS_BufInfo_t **ppstBufInfo)
{
    ...
    if (MI_SUCCESS != _MI_SYS_IMPL_UnMmapUserVirAddr(pstBufHandleData->pstBufRef))
    {
        DBG_INFO("unmmap buf to user faild\n");
        return MI_ERR_SYS_FAILED;
    }
...
void _MI_SYS_IMPL_DeinitChannel( MI_SYS_Channel_t *pstChannel)
{
    ...
    for( u32PortId=0 ; u32PortId < pstChannel->pstModDev->u32InputPortNum ; u32PortId ++)
    {
        ...
        if(MI_SUCCESS != ret)
        {
            DBG_INFO("release Input Port(%ul of module%d:%d) fail %d\n",
    ...
    for( u32PortId = 0;u32PortId < pstChannel->pstModDev->u32OutputPortNum ; u32PortId ++)
    {
        ...
        if(MI_SUCCESS != ret)
        {
            DBG_INFO("release Output Port(%ul of module%d:%d) fail %d\n",
                u32PortId, pstChannel->pstModDev->u32DevId,
                pstChannel->pstModDev->eModuleId, ret);
            MI_SYS_BUG();
        }
...
MI_S32 MI_SYS_IMPL_InjectBuf(MI_SYS_BUF_HANDLE handle , MI_SYS_ChnPort_t *pstChnInputPort)
{
    ...
    _MI_SYS_IMPL_UnMmapUserVirAddr(pstHandleData->pstBufRef);
...
{
    DBG_ENTER();
    memcpy(&pstVersion->u8Version[0] , "sky.yi mi sys v1.0" , 64);
    DBG_EXIT_OK();
    return MI_SUCCESS;
}
...
MI_S32 MI_SYS_IMPL_FlushCache(void * va, MI_U32 u32Size)
{
    MI_S32 ret = MI_ERR_SYS_FAILED;
    ...
    ret = mi_sys_buf_mgr_flushcache(va, u32Size);
    if(ret != MI_SUCCESS)
    {
        DBG_EXIT_ERR("ret = %d\n",ret);
        goto EXIT;
...
MI_S32 MI_SYS_IMPL_ChnOutputPortGetBuf(MI_SYS_ChnPort_t *pstChnPort,MI_SYS_BufInfo_t *pstBufInfo, MI_SYS_BUF_HANDLE *BufHandle)
{
    MI_SYS_BufHandleIdrData_t *pstHandleData = NULL;
    MI_SYS_BufRef_t *pstBufRef = NULL;
    MI_S32 ret = MI_ERR_SYS_FAILED;
    ...
    ret = _MI_SYS_IMPL_MmapToUserVirAddr(pstHandleData->pstBufRef);
    if(ret != MI_SUCCESS)
    {
        kfree(pstHandleData);
        mi_sys_release_bufref(pstBufRef);
    ...
    *BufHandle = idr_alloc(&g_mi_sys_buf_handle_idr, pstHandleData, MI_SYS_OUTPUT_PORT_HANDLE_START, MI_SYS_OUTPUT_PORT_HANDLE_END, GFP_KERNEL);
    if((*BufHandle) < 0)
    {
        ...
        kfree(pstHandleData);
        _MI_SYS_IMPL_UnMmapUserVirAddr(pstBufRef);
        mi_sys_release_bufref(pstBufRef);
...
MI_S32 MI_SYS_IMPL_ChnInputPortGetBuf(MI_SYS_ChnPort_t *pstChnPort ,MI_SYS_BufConf_t *pstBufConf, MI_SYS_BufInfo_t *pstBufInfo , MI_SYS_BUF_HANDLE *BufHandle , MI_S32 s32TimeOutMS)
{
    MI_SYS_BufHandleIdrData_t *pstHandleData = NULL;
    MI_SYS_BufRef_t *pstBufRef = NULL;
    MI_S32 ret = MI_ERR_SYS_FAILED;
                                            ...
                                            MI_U32 u32PortId, MI_SYS_BufConf_t *pstBufConfig, MI_BOOL *bBlockedByRateCtrl)
{
    ...
    {
        //in case user fifo enabled, we need force sideband msg to be MI_SYS_SIDEBAND_MSG_NULL
        goto alloc_buf;
    }

    if(list_empty(&pstOutputPort->stBindPeerInputPortList))
    {
        if(pstOutputPort->u32UsrFifoCount + 1 <= atomic_read(&pstOutputPort->totalOutputPortInUsedBuf))
        {
            return NULL;
        }
    }
...
alloc_buf:
    ...
    if(pstBufRef)
    {
        ...
        if(pstBufConfig->u32Flags & MI_SYS_MAP_VA)
        {
            ...
            mi_sys_buf_map_kern_va(pstBufRef->pstBufAllocation, &pstBufRef->bufinfo);
        }
        MI_SYS_BUG_ON(atomic_read(&pstBufRef->pstBufAllocation->ref_cnt) !=1);
...
/*
    ...
    mi_sys_buf_mgr_flushcache(area->addr, area->size);
*/
    vunmap(pVirtAddr);
}
