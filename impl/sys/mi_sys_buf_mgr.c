...
{
    MI_U16 u16Width = stFrameData.u16Width;
    MI_U16 u16Height = stFrameData.u16Height;
    unsigned long size = 0;

    MI_SYS_BUG_ON(!u16Width || !u16Height);

    switch(stFrameData.ePixelFormat)
    {
...
MI_U64 mi_sys_calc_buf_size_in_queue(MI_SYS_BufferQueue_t*pstQueue)
{
    ...
    list_for_each(pos, &(pstQueue->list))
    {
        ...
        {
            total_buf_size_in_queue += _mi_sys_calc_buf_size_from_frame_bufinfo(_mi_sys_get_fmt_alignment(pstBufRef->bufinfo.stFrameData.ePixelFormat),pstBufRef->bufinfo.stFrameData);
        }
        else
        {
            MI_SYS_BUG();
...
this function will be used by user,not be used by other internal VB POOL functions.
*/
static unsigned long _mi_calc_sys_frame_size(MI_SYS_BufFrameConfig_t stFrameConfig,  unsigned long alignment
...
unsigned long mi_sys_buf_mgr_get_size(MI_SYS_BufConf_t *pstBufConfig, MI_SYS_FrameSpecial_t  *pstFrameDataInfo)
{
    ...
    {
        size_to_alloc = _mi_calc_sys_frame_size(pstBufConfig->stFrameCfg,   _mi_sys_get_fmt_alignment(pstBufConfig->stFrameCfg.eFormat), pstFrameDataInfo);
    }
...
static inline int _mi_sys_try_unmap_usr_map(void *key)
{
    ...
    hash_for_each_possible(g_mi_sys_user_map_addr_hash, pUsrMapRecord, hentry, (unsigned long)key)
    {
        ...
        if(pUsrMapRecord->key == key && pUsrMapRecord->usr_process_ctx == mi_sys_buf_get_cur_user_map_ctx())
        {
            ...
            mi_sys_buf_mgr_flushcache(pUsrMapRecord->mapped_addr, pUsrMapRecord->mapped_length);
...
MI_S32 mi_sys_buf_mgr_flushcache(void *va, unsigned long len)
{
    ...
    if((long unsigned)va >= PAGE_OFFSET)
    {
        ...
        __sync_cache_range_w(va, len);//should w.not r !!!
        //step 2-a:
    ...
    else//for user space VA
    {
        ...
        if(vma->vm_file->f_op == &msys_api_mmap_fops)
        {
            ...
            and then: MI_SYS_FlushCache(xx,xx)
