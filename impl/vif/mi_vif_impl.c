...
#define MI_VIF_CHECK_PARA_RANGE(x, a, b,e) \
    {   \
        if(x < a)  \
        {   \
            DBG_EXIT_ERR("%d para too small [%d %d]!\n",x,a,b);    \
            return e;   \
        }   \
        else if(x >= b)  \
        {   \
            DBG_EXIT_ERR("%d para too big! [%d %d]\n",x,a,b);    \
            return e;   \
        }   \
    }
...
MI_S32 _mi_vif_CheckChnPortAttr(MI_VIF_CHN u32VifChn, MI_VIF_PORT u32ChnPort, MI_VIF_ChnPortAttr_t* pstAttr)
{
    ...
    if(u32ChnPort == 0)
    {
        ...
        {
            DBG_EXIT_ERR("Port 0 stCapRect.u32Height must equal stDestSize.u32Height!\n");
            return MI_ERR_VIF_INVALID_PARA;
        }

        if(pstAttr->stCapRect.u16Width != pstAttr->stDestSize.u16Width)
        {
            DBG_EXIT_ERR("Port 0 stCapRect.u32Width must equal stDestSize.u32Width!\n");
            return MI_ERR_VIF_INVALID_PARA;
        }
    }
...
static void _MI_VIF_ProcessIn(void)
{
    ...
    for(u32ChnIndex = 0; u32ChnIndex < MI_VIF_MAX_PHYCHN_NUM; u32ChnIndex++)
    {
        ...
        for(u32PortIndex = 0; u32PortIndex < MI_VIF_MAX_CHN_OUTPORT; u32PortIndex++)
        {
            ...
            while(pPortCtx->u32HalBufferHold < MI_VIF_PORT_MAX_BUFFER)
            {
                ...
                stFbInfo.nStatus       =  VIF_BUF_INVALID;
...
static void _MI_VIF_ProcessOut(void)
{
    MI_SYS_BufInfo_t* pstBufInfo;
    MHal_VIF_RingBufElm_t stFbInfo;
    MI_S32 u32ChnIndex;
...
static int _mi_vif_WorkTask(void *data)
{
    ...
    do
    {
        _MI_VIF_ProcessIn();
        _MI_VIF_ProcessOut();
        wait_for_completion_timeout(&gstVifProcessWakeup, msecs_to_jiffies(4000));
    }
...
static MI_S32 _MI_VIF_ProcOnDumpOutPortAttr(MI_SYS_DEBUG_HANDLE_t  handle, MI_U32  u32DevId, void *pUsrData)
{
    ...
    handle.OnPrintOut(handle, "           cap        dest      sel scan  fmt rate   mi_frame_status\n");
    ...
    for(u32ChnIndex = 0; u32ChnIndex < MI_VIF_MAX_PHYCHN_NUM; u32ChnIndex++)
    {
        ...
        for(u32PortIndex = 0; u32PortIndex < MI_VIF_MAX_CHN_OUTPORT; u32PortIndex++)
        {
            ...
            if(pPortCtx->bEnable == TRUE)
            {
                ...
                handle.OnPrintOut(handle, "%d    %d    %d    %d    ", pPortCtx->stChnPortAttr.eCapSel, pPortCtx->stChnPortAttr.eScanMode, pPortCtx->stChnPortAttr.ePixFormat, pPortCtx->stChnPortAttr.eFrameRate);
    ...
    for(u32ChnIndex = 0; u32ChnIndex < MI_VIF_MAX_PHYCHN_NUM; u32ChnIndex++)
    {
        ...
        for(u32PortIndex = 0; u32PortIndex < MI_VIF_MAX_CHN_OUTPORT; u32PortIndex++)
        {
            ...
            if(pPortCtx->bEnable == TRUE)
            {
                ...
                handle.OnPrintOut(handle, "(%x,%x,%x,%x)", stMhalChnStat.nFrameStartCnt, stMhalChnStat.nFrameDoneCnt, stPortStat.u32VbFail, stMhalChnStat.nDropFrameCnt);
...
MI_S32 MI_VIF_IMPL_Init(void)
{
    ...
    if(gbVifInit)
    {
        DBG_EXIT_ERR("MI_VIF already Inited, return ok !\n");
        MI_SYS_BUG();
        return MI_SUCCESS;
    }
    ...
    if(hVifHandle == NULL)
    {
        DBG_EXIT_ERR("mi_sys_RegisterDev failed!\n");
        MI_SYS_BUG();
    ...
    {
        DBG_EXIT_ERR(" MHalVifInit failed!\n");
        MI_SYS_BUG();
    ...
    {
        DBG_EXIT_ERR("run mi_vif_work_tsk failed!\n");
        MI_SYS_BUG();
...
MI_S32 MI_VIF_IMPL_DeInit(void)
{
    ...
    if(hVifHandle == NULL)
    {
        DBG_EXIT_ERR("_vif_again_Deinit failed!\n");
        MI_SYS_BUG();
    }

    if(MHAL_SUCCESS != MHal_VIF_StopMCU())
    {
        DBG_EXIT_ERR("MHal_VIF_StopMCU failed!\n");
        MI_SYS_BUG();
    }
    ...
    {
        DBG_EXIT_ERR("create MHal_VIF_Deinit failed!\n");
        MI_SYS_BUG();
    }

    if(hVifHandle == NULL)
    {
        DBG_EXIT_ERR("hVifHandle NULL!\n");
        MI_SYS_BUG();
    }
    else
    {
...
MI_S32 MI_VIF_IMPL_SetDevAttr(MI_VIF_DEV u32VifDev, MI_VIF_DevAttr_t *pstDevAttr)
{
    ...
    {
        DBG_EXIT_ERR("dev %d has enabled!\n", u32VifDev);
    ...
    if(MHAL_SUCCESS != MHal_VIF_DevSetConfig(u32VifDev, &stDevAttr))
    {
        DBG_EXIT_ERR("DevSetConfig failed!\n");
        MI_SYS_BUG();
        return E_MI_ERR_FAILED;
    }
...
MI_S32 MI_VIF_IMPL_DisableDev(MI_VIF_DEV u32VifDev)
{
    ...
    if(pDevctx->bEnable == TRUE)
    {
        ...
        {
            DBG_EXIT_ERR("DisableDev failed!\n");
            return E_MI_ERR_FAILED;
        }
...
MI_S32 MI_VIF_IMPL_SetChnPortAttr(MI_VIF_CHN u32VifChn, MI_VIF_PORT u32ChnPort, MI_VIF_ChnPortAttr_t *pstAttr)
{
    ...
    if(pDevctx->bEnable == FALSE)
    {
        DBG_EXIT_ERR("dev %d not enabled!\n", u32VifDev);
        return MI_ERR_VIF_FAILED_DEVNOTENABLE;
    }

    if(pPortCtx->bEnable == TRUE)
    {
        DBG_EXIT_ERR("chn %d port :%d has enabled!\n", u32VifChn, u32ChnPort);
    ...
    if(u32ChnPort == 0)
    {
        ...
        {
            DBG_EXIT_ERR("ChnSetConfig failed!\n");
            MI_SYS_BUG();
    ...
    else if(u32ChnPort == 1)
    {
        ...
        {
            DBG_EXIT_ERR("SubChnSetConfig failed!\n");
            MI_SYS_BUG();
            s32Ret = E_MI_ERR_FAILED;
            goto done;
        }
...
MI_S32 MI_VIF_IMPL_EnableChnPort(MI_VIF_CHN u32VifChn, MI_VIF_PORT u32ChnPort)
{
    ...
    if(pPortCtx->bEnable == FALSE)
    {
        ...
        else if(u32ChnPort == 1)
        {
            ...
            {
                DBG_EXIT_ERR("SubChnEnable failed!\n");
                s32Ret = E_MI_ERR_FAILED;
                goto done;
            }
        }
...
MI_S32 MI_VIF_IMPL_DisableChnPort(MI_VIF_CHN u32VifChn, MI_VIF_PORT u32ChnPort)
{
    ...
    if(pPortCtx->bEnable == TRUE)
    {
        ...
        if(u32ChnPort == 0)
        {
            ...
            {
                DBG_EXIT_ERR("ChnDisable failed!\n");
                MI_SYS_BUG();
                s32Ret = E_MI_ERR_FAILED;
                goto done;
            }
        ...
        else if(u32ChnPort == 1)
        {
            ...
            {
                DBG_EXIT_ERR("SubChnDisable failed!\n");
                MI_SYS_BUG();
                s32Ret = E_MI_ERR_FAILED;
                goto done;
            }
