...
typedef struct MI_VDEC_StatDecCnt_s
{
    MI_BOOL bStat;           // switch of Stat all chn pts interval
    MI_VDEC_StatFrameTimer_t _stStatFrmTimer;
    MI_U16  u16LastDecCnt[MI_VDEC_MAX_CHN_NUM];
    MI_U16  u16CurDecCnt[MI_VDEC_MAX_CHN_NUM];

} MI_VDEC_StatDecCnt_t;
...
static MI_VDEC_StatDecCnt_t _stStatDecCnt;
...
void _MI_VDEC_StatOutputTimeIntvl(MI_VDEC_CHN VdecChn, MI_SYS_BufInfo_t *pstBufInfo);
#endif
...
#elif LINUX_VERSION_CODE == KERNEL_VERSION(3,18,30)
            wait_event_interruptible_timeout(_stResMgr.stInjWaitQueueHead, FALSE, msecs_to_jiffies(2));
#endif
...
#if (defined MI_SYS_PROC_FS_DEBUG) && (MI_VDEC_PROCFS_DEBUG == 1)
            _MI_VDEC_StatOutputTimeIntvl(u16Chn, pstBufInfo);
#endif
...
#elif LINUX_VERSION_CODE == KERNEL_VERSION(3,18,30)
            wait_event_interruptible_timeout(_stResMgr.stPutWaitQueueHead, FALSE, msecs_to_jiffies(2));
#endif
        }
    }
    return MI_SUCCESS;
...
MI_BOOL _MI_VDEC_IMPL_JpegDecoder(MI_VDEC_CHN VdecChn, MI_SYS_BufInfo_t *pstBufInfo, MI_SYS_BufInfo_t *pstPutBufInfo)
{
    MI_U32 u32EsBufSize = 0;
    MI_U8 u8TryDecCnt = 0;
    static MI_U8 u8DumpCount = 0;
    MI_U32 u32Offset = 0;
    MI_U8 *pu8VdecBufAddr = NULL;
    MI_U8 *pu8BitStreamVirAddr = NULL;
    mi_vdec_BufInfo_t stBufInfo;
    mi_vdec_DispFrame_t stGetJpegDispFrm;
#define _MI_VDEC_TRY_DEC_COUNT 10
#define _MI_VDEC_TRY_SLEEP_TIME 10 //ms

    if (!(pstPutBufInfo && pstBufInfo))
    {
        DBG_ERR("pstBufInfo:%p, pstPutBufInfo:%p\n", pstBufInfo, pstPutBufInfo);
        return FALSE;
    }

    u32EsBufSize = pstBufInfo->stRawData.u32BufSize;

    pstBufInfo->stRawData.pVirAddr = mi_sys_Vmap(
            pstBufInfo->stRawData.phyAddr,
            _ALIGN_BYTE_(pstBufInfo->stRawData.u32BufSize, MI_VDEC_PAGE_SIZE),
            FALSE);

    memset(&stBufInfo, 0x0, sizeof(mi_vdec_BufInfo_t));
    stBufInfo.u32BufSize = u32EsBufSize;
    if (stBufInfo.u32BufSize >= _stResMgr.astChnInfo[VdecChn].stEsMemInfo.u32BufSize)
    {
        DBG_ERR("Chn(%d) Out Of Es Buffer Limited, Drop Es. Push Data Size:%d, Es Buffer Size:%d\n", VdecChn,
            stBufInfo.u32BufSize, _stResMgr.astChnInfo[VdecChn].stEsMemInfo.u32BufSize);
    ...
    u32Offset = stBufInfo.phyAddr - _stResMgr.astChnInfo[VdecChn].stEsMemInfo.phyAddr;
    pu8BitStreamVirAddr = (MI_U8 *)(_stResMgr.astChnInfo[VdecChn].stEsMemInfo.pVirAddr) + u32Offset;
    memcpy(pu8BitStreamVirAddr, pstBufInfo->stRawData.pVirAddr, u32EsBufSize);
    mi_sys_UnVmap(pstBufInfo->stRawData.pVirAddr);
    ...
    do
    {
         memset(&stGetJpegDispFrm, 0x0, sizeof(mi_vdec_DispFrame_t));
         if (MI_SUCCESS != _MI_VDEC_IMPL_GetFrame(VdecChn, &stGetJpegDispFrm))
         {
            msleep(_MI_VDEC_TRY_SLEEP_TIME);//ms
            u8TryDecCnt++;
         }
         else
         {
            u32Offset = stGetJpegDispFrm.stFrmInfo.phyLumaAddr - _stResMgr.astChnInfo[VdecChn].stFrameMemInfo.phyAddr;
            pu8VdecBufAddr = _stResMgr.astChnInfo[VdecChn].stFrameMemInfo.pVirAddr + u32Offset;

            DBG_INFO("phyLumaAddr[%llx] phyChromaAddr[%llx] u32Offset[%u]  stFrameMemInfo.pVirAddr[%p] pu8VdecBufAddr[%p]\n", stGetJpegDispFrm.stFrmInfo.phyLumaAddr, stGetJpegDispFrm.stFrmInfo.phyChromaAddr, u32Offset,  _stResMgr.astChnInfo[VdecChn].stFrameMemInfo.pVirAddr, pu8VdecBufAddr);
            if (_stDumpParams.bDumpFB[VdecChn])
            {
                _MI_VDEC_IMPL_DebugDumpJpegDecImage(pu8VdecBufAddr, stGetJpegDispFrm.stFrmInfo.u16Width, stGetJpegDispFrm.stFrmInfo.u16Height, stGetJpegDispFrm.stFrmInfo.u16Pitch, VdecChn);

                u8DumpCount++;
                if(u8DumpCount > 2)
                {
                    _stDumpParams.bDumpFB[VdecChn] = FALSE;
                    u8DumpCount = 0;
                }
            }
    ...
    }while(u8TryDecCnt < _MI_VDEC_TRY_DEC_COUNT);

    if(u8TryDecCnt < _MI_VDEC_TRY_DEC_COUNT)
    {
        pstPutBufInfo->stFrameData.ePixelFormat = E_MI_SYS_PIXEL_FRAME_YUV422_YUYV;
        pstPutBufInfo->stFrameData.u16Width = stGetJpegDispFrm.stFrmInfo.u16Width;
        pstPutBufInfo->stFrameData.u16Height = stGetJpegDispFrm.stFrmInfo.u16Height;
        pstPutBufInfo->stFrameData.u32Stride[0] = stGetJpegDispFrm.stFrmInfo.u16Pitch * 2;
        pstPutBufInfo->stFrameData.phyAddr[0] = stGetJpegDispFrm.stFrmInfo.phyLumaAddr;
        _MI_VDEC_IMPL_RlsFrame(VdecChn, &stGetJpegDispFrm);
        return TRUE;
    }
    else
    {
...
{
    MI_BOOL bBlockedByRateCtrl = FALSE;
    MI_SYS_BufConf_t stJpegBufCfg;
    MI_SYS_BufInfo_t *pstPutBufInfo = NULL;
    MI_SYS_BufInfo_t *pstInjBufInfo = NULL;
    mi_vdec_DispFrame_t stGetJpegDispFrm;

    DBG_INFO("Create Put Frame Task Done\n");

    memset(&stJpegBufCfg, 0x0, sizeof(MI_SYS_BufConf_t));
    stJpegBufCfg.eBufType = E_MI_SYS_BUFDATA_FRAME;
    stJpegBufCfg.u64TargetPts = MI_SYS_INVALID_PTS;

    while (_stResMgr.bPutFrmJpegTaskRun)
    {
        MI_U16 u16Chn = 0;
        MI_BOOL bPutFrm = FALSE;
        MI_BOOL bFinishBuf = FALSE;
        for (u16Chn = 0; u16Chn < MI_VDEC_MAX_CHN_NUM; ++u16Chn)
        {
            pstInjBufInfo = NULL;
            pstPutBufInfo = NULL;
            ...
            stJpegBufCfg.stFrameCfg.eFormat = E_MI_SYS_PIXEL_FRAME_YUV422_YUYV;
            stJpegBufCfg.stFrameCfg.eFrameScanMode = E_MI_SYS_FRAME_SCAN_MODE_PROGRESSIVE;
            stJpegBufCfg.stFrameCfg.u16Height = _ALIGN_BYTE_(_stResMgr.astChnInfo[u16Chn].stChnAttr.u32PicHeight, MI_VDEC_IMAGE_PITCH_ALIGN_SIZE);
            stJpegBufCfg.stFrameCfg.u16Width = _ALIGN_BYTE_(_stResMgr.astChnInfo[u16Chn].stChnAttr.u32PicWidth, MI_VDEC_IMAGE_PITCH_ALIGN_SIZE);

            pstPutBufInfo = mi_sys_GetOutputPortBuf(_stResMgr.hVdecDev, u16Chn, 0, &stJpegBufCfg ,&bBlockedByRateCtrl);
            if (pstPutBufInfo == NULL)
            {
                ///no output buffer
                if ((E_MI_VDEC_DISPLAY_MODE_PREVIEW == _stResMgr.astChnInfo[u16Chn].eDisplayMode) || bBlockedByRateCtrl)
                {
                    ///maybe blocked by others, preview need to drop frame
                    DOWN(&_stResMgr.semChnLock[u16Chn]);
                    pstInjBufInfo = mi_sys_GetInputPortBuf(_stResMgr.hVdecDev, u16Chn, 0, MI_SYS_MAP_VA);
                    if (pstInjBufInfo != NULL)
                    {
                        mi_sys_FinishBuf(pstInjBufInfo);
                        bFinishBuf = TRUE;
                    }
                    UP(&_stResMgr.semChnLock[u16Chn]);
                }
            ...
            MI_SYS_BUG_ON(pstPutBufInfo->eBufType != E_MI_SYS_BUFDATA_FRAME);
            memset(&stGetJpegDispFrm, 0x0, sizeof(mi_vdec_DispFrame_t));
            if ((pstPutBufInfo->stFrameData.u16Height < stJpegBufCfg.stFrameCfg.u16Height)
                || (pstPutBufInfo->stFrameData.u16Width < stJpegBufCfg.stFrameCfg.u16Width))
            {
                DBG_ERR("pstBufInfo->stFrameData.u16Height < stJpegBufCfg.stFrameCfg.u16Height ||"
                    "pstBufInfo->stFrameData.u16Width < stJpegBufCfg.stFrameCfg.u16Width");
                mi_sys_RewindBuf(pstPutBufInfo);
                continue;
            }
            ...
            if (pstInjBufInfo != NULL)
            {
                if (TRUE == _MI_VDEC_IMPL_JpegDecoder(u16Chn, pstInjBufInfo, pstPutBufInfo))
                {
                    mi_sys_FinishBuf(pstInjBufInfo);
                    bFinishBuf = TRUE;
                }
                ...
                {
                    mi_sys_RewindBuf(pstInjBufInfo);
                }
            }
            ...
            if (bFinishBuf)
            {
                mi_sys_FinishBuf(pstPutBufInfo);
            }
            else
            {
                mi_sys_RewindBuf(pstPutBufInfo);
        ...
        else
        {
            ///sleep
#if LINUX_VERSION_CODE == KERNEL_VERSION(3,10,40)
            interruptible_sleep_on_timeout(&_stResMgr.stPutWaitJpegQueueHead, 10);
#elif LINUX_VERSION_CODE == KERNEL_VERSION(3,18,30)
            wait_event_interruptible_timeout(_stResMgr.stPutWaitJpegQueueHead, FALSE, msecs_to_jiffies(10));
#endif
        }
    }
    return MI_SUCCESS;
...
static MI_S32 _MI_VDEC_IMPL_StartChn(MI_VDEC_CHN VdecChn)
{
    ...
    if (_stDumpParams.u32DisableMFDec)
    {
    ...
    {
        //inter buffer ==>  MaxProWidth*MaxProHeight *2*3(AC) + MaxProWidth*MaxProHeight*2*3/64(DC) + 8K + (64K + 128)
        stInitParams.SysConfig.u32DrvProcBufSize =
            MI_VDEC_SUPPORT_MAX_PROGRESSICE_JPEG_WIDTH*MI_VDEC_SUPPORT_MAX_PROGRESSICE_JPEG_HEIGHT*2*3 +
            (MI_VDEC_SUPPORT_MAX_PROGRESSICE_JPEG_WIDTH*MI_VDEC_SUPPORT_MAX_PROGRESSICE_JPEG_HEIGHT*2*3)/64 +
            8*1024 + (64*1024 + 128);
    }
    else
    {
...
static MI_S32 _MI_VDEC_ProcOnDumpChannelAttr(MI_SYS_DEBUG_HANDLE_t  handle, MI_U32  u32DevId, void *pUsrData)
{
    ...
    for(VdecChn = 0; VdecChn < MI_VDEC_MAX_CHN_NUM; VdecChn++)
    {
        ...
        handle.OnPrintOut(handle, "%5u  %9d  %5u  %6u  %8u  %8u  %9d  %10d  %9d\n",\
            VdecChn, pstChnAttr->eCodecType, pstChnAttr->u32PicWidth, pstChnAttr->u32PicHeight, pstChnAttr->u32BufSize,\
            pstChnAttr->u32Priority, pstChnAttr->eVideoMode, pstChnAttr->stVdecJpegAttr.eJpegFormat, \
            pstChnAttr->stVdecVideoAttr.u32RefFrameNum);
    }

    handle.OnPrintOut(handle, "\n------------------------------------CHN PARAM Info--------------------------------------\n");
    handle.OnPrintOut(handle, "%5s  %7s  %11s  %11s  %11s\n", \
    ...
    for(VdecChn = 0; VdecChn < MI_VDEC_MAX_CHN_NUM; VdecChn++)
    {
        ...
        eDisplayMode = _stResMgr.astChnInfo[VdecChn].eDisplayMode;
        handle.OnPrintOut(handle, "%5u  %7d  %11d  %11d  %11d\n",\
            VdecChn, pstChnParam->eDecMode, pstChnParam->eOutputOrder, pstChnParam->eVideoFormat, eDisplayMode);
    }
    ...
    for(VdecChn = 0; VdecChn < MI_VDEC_MAX_CHN_NUM; VdecChn++)
    {
        ...
        handle.OnPrintOut(handle, "%5u  %6d  %9d  %13u  %11u  %10u  %11u  %10u\n",\
            VdecChn, stChnStat.bChnStart, stChnStat.eCodecType, stChnStat.u32LeftStreamBytes, \
    ...
    if(_stStatDecCnt.bStat)
    {
        handle.OnPrintOut(handle, "\n------------------------------------CHN STATE 2-----------------------------------------\n");
        handle.OnPrintOut(handle, "%5s  %12s\n", "ChnID","DecCntLastSec");
        ...
        for(VdecChn = 0; VdecChn < MI_VDEC_MAX_CHN_NUM; VdecChn++)
        {
            ...
            handle.OnPrintOut(handle, "%5u  %12u\n", VdecChn, _stStatDecCnt.u16LastDecCnt[VdecChn]);
        }
    }
...
void _MI_VDEC_StatOutputTimeIntvl(MI_VDEC_CHN VdecChn, MI_SYS_BufInfo_t *pstBufInfo)
{
    ...
    if(_stStatDecCnt.bStat)
    {
        _stStatDecCnt.u16CurDecCnt[VdecChn] += 1;
    }
...
void _MI_VDEC_StatDecCnt(unsigned long arg)
{
    MI_VDEC_CHN VdecChn = 0;

    for(VdecChn = 0; VdecChn < MI_VDEC_MAX_CHN_NUM; VdecChn ++)
    {
        if(_stResMgr.astChnInfo[VdecChn].bStart && _stStatDecCnt.bStat)
        {
            _stStatDecCnt.u16LastDecCnt[VdecChn] = _stStatDecCnt.u16CurDecCnt[VdecChn];
        }

        _stStatDecCnt.u16CurDecCnt[VdecChn] = 0;
    }

    mod_timer(&_stStatDecCnt._stStatFrmTimer.Timer, jiffies+HZ);//set 1s timeout
}
...
static MI_S32 _MI_VDEC_StatDecCntLastSec(MI_SYS_DEBUG_HANDLE_t handle, MI_U32 u32DevId, MI_U8 argc, MI_U8 **argv, void *pUsrData)
{
    if(2 > argc)
    {
        handle.OnPrintOut(handle, "Unsupport command: %s \n", argv[0]);
    ...
    {
        _stStatDecCnt.bStat = TRUE;
        memset(&_stStatDecCnt.u16LastDecCnt, 0, sizeof(MI_U16) * MI_VDEC_MAX_CHN_NUM);
        memset(&_stStatDecCnt.u16CurDecCnt,  0, sizeof(MI_U16) * MI_VDEC_MAX_CHN_NUM);
    }
    else if(!strncasecmp(argv[1], "off", strlen("off")))
    {
        _stStatDecCnt.bStat = FALSE;
    ...
    if(FALSE == _stStatDecCnt._stStatFrmTimer.bCreate && TRUE == _stStatDecCnt.bStat)
    {
        init_timer(&_stStatDecCnt._stStatFrmTimer.Timer);
        _stStatDecCnt._stStatFrmTimer.Timer.function = _MI_VDEC_StatDecCnt;
        add_timer(&_stStatDecCnt._stStatFrmTimer.Timer);
        mod_timer(&_stStatDecCnt._stStatFrmTimer.Timer, jiffies+HZ);//set 1s timeout

        _stStatDecCnt._stStatFrmTimer.bCreate = TRUE;
    }

    if(TRUE == _stStatDecCnt._stStatFrmTimer.bCreate && FALSE == _stStatDecCnt.bStat)
    {
        del_timer(&_stStatDecCnt._stStatFrmTimer.Timer);
        _stStatDecCnt._stStatFrmTimer.bCreate = FALSE;
    }
...
GET_LX_FINISH:
    ...
    if (MI_SUCCESS != mi_sys_MMA_Alloc(_au8MmaHeapName[_stResMgr.stCpuMem.u8Miu],
    ...
    mi_sys_RegistCommand("StatDecCntLastSec",   1, _MI_VDEC_StatDecCntLastSec,          _stResMgr.hVdecDev);
    ...
    sched_setscheduler(_stResMgr.ptskInjTask, SCHED_RR, &param);

    param.sched_priority = 99;
    sched_setscheduler(_stResMgr.ptskPutFrmTask, SCHED_RR, &param);

    param.sched_priority = 99;
    sched_setscheduler(_stResMgr.ptskPutJpegFrmTask, SCHED_RR, &param);
...
MI_S32 MI_VDEC_IMPL_DeInit(void)
{
    ...
    if(_stStatDecCnt._stStatFrmTimer.bCreate)
    {
        del_timer(&_stStatDecCnt._stStatFrmTimer.Timer);
        _stStatDecCnt._stStatFrmTimer.bCreate = FALSE;
    }
...
/* h264 hw buffer 1M, h265 hw buffer 2M
*  1x400w   h265  2560*1440*4*1.5+2M = 26M
    2x1080p  h264  1920*1080*4*1.5+1M = 26M
    4x720p    h264  1280*720*4*1.5+1M = 28M
    8xD1       h264  720*576*4*1.5+1M = 32M
*/
    ...
    ///alloc frame buffer
    if (4 > pstChnAttr->stVdecVideoAttr.u32RefFrameNum)
    {
        pstChnAttr->stVdecVideoAttr.u32RefFrameNum = 4;
    }
    ...
    if (pstChnAttr->eCodecType == E_MI_VDEC_CODEC_TYPE_JPEG)
    {
        ...
        _stResMgr.astChnInfo[VdecChn].stFrameMemInfo.u32BufSize =
            (_ALIGN_BYTE_(pstChnAttr->u32PicWidth, MI_VDEC_IMAGE_PITCH_ALIGN_SIZE)
            * _ALIGN_BYTE_(pstChnAttr->u32PicHeight, MI_VDEC_IMAGE_PITCH_ALIGN_SIZE) * 2)
            * pstChnAttr->stVdecVideoAttr.u32RefFrameNum;
...
MI_S32 MI_VDEC_IMPL_StopChn(MI_VDEC_CHN VdecChn)
{
    ...
    MApi_VDEC_EX_Exit(&(_stResMgr.astChnInfo[VdecChn].stVDECStreamId));
    _stResMgr.astChnInfo[VdecChn].bStart = FALSE;
    _stStatDecCnt.u16LastDecCnt[VdecChn] = 0;
#if (defined MI_SYS_PROC_FS_DEBUG) && (MI_VDEC_PROCFS_DEBUG == 1)
    UP(&(_stVdecProcInfo.stProcChnLock[VdecChn]));
#endif
    atomic_inc(&(_stResMgr.u64FastChnId[VdecChn]));
...
void _MI_VDEC_IMPL_DebugDumpFrameBuffer(MI_U32 u32Chn, MI_SYS_BufInfo_t *pstBufInfo)
{
    ...
    if (bDump)
    {
        ...
        sprintf(au8FileName, "%s/chn_%d_dump_vdec[%d_%d_%d]_%d_chrome.yuv", _stDumpParams.au8DumpPath, u32Chn,
            pstDispFrm->stFrmInfo.u16Width, pstDispFrm->stFrmInfo.u16Height, u32Pitch, u32GetFrm[u32Chn]);
