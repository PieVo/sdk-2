...
static MI_S32 _MI_AO_IMPL_OnDumpDevAttr(MI_SYS_DEBUG_HANDLE_t  handle, MI_U32  u32DevId, void *pUsrData)
{
    ...
    MI_U32 u32WriteTotalFrmCnt = pstAoDevInfo->astChanInfo[0].u32WriteTotalFrmCnt;
    ...
    handle.OnPrintOut(handle,"%5d  %8d  %13d \n", bMuteEnable, s32VolumeDb, u32WriteTotalFrmCnt);
...
static MI_S32 _MI_AO_IMPL_OnDumpChannelAttr(MI_SYS_DEBUG_HANDLE_t  handle, MI_U32  u32DevId, void *pUsrData)
{
    ...
    handle.OnPrintOut(handle,"AoDev  AoChn  bAnr            Speed  Intensity  SmoothLevel\n");
    handle.OnPrintOut(handle,"%5d  %5d  %4d  %15s  %9d  %11d\n",AoDevId, AoChn, stAoVqeConfig.bAnrOpen, szAnrSpeed,
                                                                   stAoVqeConfig.stAnrCfg.u32NrIntensity,
                                                                   stAoVqeConfig.stAnrCfg.u32NrSmoothLevel);

    // Eq Status
    handle.OnPrintOut(handle,"-----AO CHN%d Eq TATUS------------------------------------------------------\n", AoChn);
    handle.OnPrintOut(handle,"AoDev  AoChn  bEq  100Hz  200Hz  250Hz  350Hz  500Hz  800Hz  1.2KHz  2.5KHz  4KHz  8KHz  \n");
    handle.OnPrintOut(handle,"%5d  %5d  %3d  %5d  %5d  %5d  %5d  %5d  %5d  %6d  %6d  %4d  %4d  \n",
        AoDevId, AoChn, stAoVqeConfig.bEqOpen,
    ...
    handle.OnPrintOut(handle,"AoDev  AoChn  bHpf  HpfFreq  \n");
    handle.OnPrintOut(handle,"%5d  %5d  %4d  %7d  \n",AoDevId, AoChn, stAoVqeConfig.bHpfOpen, stAoVqeConfig.stHpfCfg.eHpfFreq);

    // Agc Status
    handle.OnPrintOut(handle,"-----AO CHN%d Agc STATUS------------------------------------------------------\n", AoChn);
    handle.OnPrintOut(handle,"AoDev  AoChn  bAgc  AttackTime  ReleaseTime  CompressionRatio  DropGainMax  \n");
    handle.OnPrintOut(handle,"%5d  %5d  %4d  %10d  %11d  %16d  %11d   \n", AoDevId, AoChn, stAoVqeConfig.bAgcOpen,
...
int _MI_AO_WriteDataThread(void* data)
{
    ...
    while(!MI_AO_ThreadShouldStop())
    {
        ...
        mi_sys_WaitOnInputTaskAvailable(hDevSysHandle, 3);
        ...
        if(NULL != pstInputBufInfo)
        {
            ...
            if(s32WriteActualSize >= 0)
            {
                ...
                {
                    _gastAoTimerInfo[AoutDevId].u32CurrentSize = u32GetTotalSize;
                }
    ...
    DBG_INFO("Total write size is %d \n", u32GetTotalSize);
...
MI_S32 MI_AO_IMPL_Disable(MI_AUDIO_DEV AoDevId)
{
    ...
    // 1.0 according AO device ID， stop _MI_AO_WriteDataThread
