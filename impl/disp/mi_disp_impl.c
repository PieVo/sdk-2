...
static stDISPStatFrame_t gstDISPStatFrameInfo[MI_DISP_LAYER_MAX][MI_DISP_INPUTPORT_MAX];
static stDISPCheckFramePts_t gstDISPCheckFramePts[MI_DISP_LAYER_MAX][MI_DISP_INPUTPORT_MAX];
static stDISPStatFrameTimer_t gstDISPStatFrameTimer;

void _mi_disp_StatFrameIntervaltime(MI_SYS_BufInfo_t *pstBufInfo, MI_U8 DispChn ,MI_U8 InPortID);
#endif
...
MI_S32 MI_DISP_IMPL_Enable(MI_DISP_DEV DispDev)
{
    ...
    DISP_DBG_INFO("Enable Display Dev %d !\n", DispDev);
...
{
    MI_S32 s32Ret = E_MI_ERR_FAILED;
    DISP_DBG_INFO("Disable Display Dev %d !\n", DispDev);
    MI_DISP_CHECK_INVAILDDEV(DispDev);
...
MI_S32 MI_DISP_IMPL_SetPubAttr(MI_DISP_DEV DispDev, const MI_DISP_PubAttr_t *pstPubAttr)
{
    ...
    DISP_DBG_INFO("Set Display dev %d, Public Attrs !\n", DispDev);
    MI_DISP_CHECK_INVAILDDEV(DispDev);
...
MI_S32 MI_DISP_IMPL_GetPubAttr(MI_DISP_DEV DispDev, MI_DISP_PubAttr_t *pstPubAttr)
{
    ...
    MHAL_DISP_DeviceTimingInfo_t stDevTimingInfo;
    memset(&stDevTimingInfo, 0, sizeof(MHAL_DISP_DeviceTimingInfo_t));
    DISP_DBG_INFO("Get Display dev %d, Public Attrs !\n", DispDev);
    MI_DISP_CHECK_INVAILDDEV(DispDev);
    MI_DISP_CHECK_NULLPOINTER(pstPubAttr);
...
{
    MI_S32 s32Ret = E_MI_ERR_FAILED;
    DISP_DBG_INFO("Attach Dev %d to Dest Dev %d !\n", DispSrcDev, DispDstDev);
...
MI_S32 MI_DISP_IMPL_EnableVideoLayer(MI_DISP_LAYER DispLayer)
{
    ...
    MI_U8 u8PortCount = 0;
    MI_U8 u8DevID = 0;
    DISP_DBG_INFO("MI_DISP_EnableVideoLayer, VideoLayer = %d !\n", DispLayer);
...
#if MI_DISP_ENABLE_CUS_ALLOCATOR
...
#endif
    DISP_DBG_INFO("MI_DISP_IMPL_DisableVideoLayer, VideoLayer = %d !\n", DispLayer);
    MI_DISP_CHECK_INVAILDLAYER(DispLayer);
    u8DevID = astLayerParams[DispLayer].u8BindedDevID;
    MI_DISP_CHECK_INVAILDDEV(u8DevID);
...
MI_S32 MI_DISP_IMPL_SetVideoLayerAttr(MI_DISP_LAYER DispLayer, const MI_DISP_VideoLayerAttr_t *pstLayerAttr)
{
    ...
    int i = 0;
    DISP_DBG_INFO("MI_DISP_SetVideoLayerAttr!\n");
    MI_DISP_CHECK_INVAILDLAYER(DispLayer);
...
UP_DEV_LAYER_MUTEX:
    //up(&(astLayerParams[DispLayer].stLayerMutex));
    //up(&(astDevStatus[astLayerParams[DispLayer].u8BindedDevID].stDevMutex));
...
{
    MI_S32 s32Ret = E_MI_ERR_FAILED;
    DISP_DBG_INFO("MI_DISP_GetVideoLayerAttr!\n");
    MI_DISP_CHECK_INVAILDLAYER(DispLayer);
    MI_DISP_CHECK_NULLPOINTER(pstLayerAttr);
#if 0
    down(&(astDevStatus[astLayerParams[DispLayer].u8BindedDevID].stDevMutex));
...
{
    MI_S32 s32Ret = E_MI_ERR_FAILED;
    DISP_DBG_INFO("DispLayer = %u, DispDev= %u!\n", DispLayer,DispDev );
    MI_DISP_CHECK_INVAILDDEV(DispDev);
    MI_DISP_CHECK_INVAILDLAYER(DispLayer);
#if 0
    MI_DISP_DOWN(&(astDevStatus[DispDev].stDevMutex));
...
MI_S32 MI_DISP_IMPL_UnBindVideoLayer(MI_DISP_LAYER DispLayer, MI_DISP_DEV DispDev)
{
    ...
    DISP_DBG_INFO("MI_DISP_UnBindVideoLayer!\n");
    MI_DISP_CHECK_INVAILDDEV(DispDev);
    MI_DISP_CHECK_INVAILDLAYER(DispLayer);
...
{
    MI_S32 s32Ret = E_MI_ERR_FAILED;
    DISP_DBG_INFO("MI_DISP_SetPlayToleration!\n");
    MI_DISP_CHECK_INVAILDLAYER(DispLayer);
    //down(&(astLayerParams[DispLayer].stLayerMutex));
    MI_DISP_GetVarMutex();
...
{
    MI_S32 s32Ret = E_MI_ERR_FAILED;
    DISP_DBG_INFO("MI_DISP_GetPlayToleration!\n");
    MI_DISP_CHECK_INVAILDLAYER(DispLayer);
...
MI_S32 MI_DISP_IMPL_GetScreenFrame(MI_DISP_LAYER DispLayer, MI_DISP_VideoFrame_t *pstVFrame)
{
    ...
    DISP_DBG_INFO("MI_DISP_GetScreenFrame!\n");
    MI_DISP_CHECK_INVAILDLAYER(DispLayer);
...
{
    MI_S32 s32Ret = E_MI_ERR_FAILED;
    DISP_DBG_INFO("MI_DISP_ReleaseScreenFrame!\n");
    MI_DISP_CHECK_INVAILDLAYER(DispLayer);
    MI_DISP_CHECK_NULLPOINTER(pstVFrame);
...
MI_S32 MI_DISP_IMPL_SetInputPortAttr(MI_DISP_LAYER DispLayer, MI_DISP_INPUTPORT LayerInputPort, const MI_DISP_InputPortAttr_t *pstInputPortAttr)
{
    ...
    DISP_DBG_INFO("MI_DISP_SetInputPortAttr!\n");
    MI_DISP_CHECK_INVAILDLAYER(DispLayer);
    MI_DISP_CHECK_INVAILDINPUTPORT(LayerInputPort);
...
{
    MI_S32 s32Ret = E_MI_ERR_FAILED;
    DISP_DBG_INFO("MI_DISP_GetInputPortAttr!\n");
    MI_DISP_CHECK_INVAILDLAYER(DispLayer);
    MI_DISP_CHECK_INVAILDINPUTPORT(LayerInputPort);
    MI_DISP_CHECK_NULLPOINTER(pstInputPortAttr);
...
MI_S32 MI_DISP_IMPL_EnableInputPort (MI_DISP_LAYER DispLayer, MI_DISP_INPUTPORT LayerInputPort)
{
    ...
    DISP_DBG_INFO("MI_DISP_EnableInputPort!\n");
    MI_DISP_GetVarMutex();
...
MI_S32 MI_DISP_IMPL_SetInputPortDispPos(MI_DISP_LAYER DispLayer, MI_DISP_INPUTPORT LayerInputPort, const MI_DISP_Position_t *pstDispPos)
{
    MI_S32 s32Ret = E_MI_ERR_FAILED;
    MHAL_DISP_InputPortAttr_t stHalInputPortAttr;
    DISP_DBG_INFO("MI_DISP_SetInputPortDispPos!\n");
    MI_DISP_CHECK_INVAILDLAYER(DispLayer);
    MI_DISP_CHECK_INVAILDINPUTPORT(LayerInputPort);
...
{
    MI_S32 s32Ret = E_MI_ERR_FAILED;
    DISP_DBG_INFO("MI_DISP_ShowInputPort!\n");
    MI_DISP_CHECK_INVAILDLAYER(DispLayer);
    MI_DISP_CHECK_INVAILDINPUTPORT(LayerInputPort);
...
{
    MI_S32 s32Ret = E_MI_ERR_FAILED;
    DISP_DBG_INFO("MI_DISP_HideInputPort!\n");
    MI_DISP_CHECK_INVAILDLAYER(DispLayer);
    MI_DISP_CHECK_INVAILDINPUTPORT(LayerInputPort);
    //down(&(astLayerParams[DispLayer].astPortStatus[LayerInputPort].stPortMutex));
    MI_DISP_GetVarMutex();
...
{
    MI_S32 s32Ret = E_MI_ERR_FAILED;
    DISP_DBG_INFO("MI_DISP_GetInputPortPts!\n");
    MI_DISP_CHECK_INVAILDLAYER(DispLayer);
    MI_DISP_CHECK_INVAILDINPUTPORT(LayerInputPort);
...
{
    MI_S32 s32Ret = E_MI_ERR_FAILED;
    DISP_DBG_INFO("MI_DISP_SetInputPortSyncMode!\n");
    MI_DISP_CHECK_INVAILDLAYER(DispLayer);
    MI_DISP_CHECK_INVAILDINPUTPORT(LayerInputPort);
...
{
    MI_S32 s32Ret = E_MI_ERR_FAILED;
    DISP_DBG_INFO("MI_DISP_IMPL_QueryInputPortStat!\n");
    MI_DISP_CHECK_INVAILDLAYER(DispLayer);
    MI_DISP_CHECK_INVAILDINPUTPORT(LayerInputPort);
...
MI_S32 MI_DISP_IMPL_SetZoomInWindow(MI_DISP_LAYER DispLayer, MI_DISP_INPUTPORT LayerInputPort, MI_DISP_VidWinRect_t* pstCropWin)
{
    ...
    DISP_DBG_INFO("MI_DISP_IMPL_SetZoomInWindow!\n");
    MI_DISP_CHECK_INVAILDLAYER(DispLayer);
    MI_DISP_CHECK_INVAILDINPUTPORT(LayerInputPort);
...
{
    MI_S32 s32Ret = E_MI_ERR_FAILED;
    DISP_DBG_INFO("MI_DISP_GetVgaParam !!!\n");
    MI_DISP_CHECK_INVAILDDEV(DispDev);
...
MI_S32 MI_DISP_IMPL_SetVgaParam(MI_DISP_DEV DispDev, MI_DISP_VgaParam_t *pstVgaParam)
{
    ...
    DISP_DBG_INFO("MI_DISP_SetVgaParam !!!\n");
    MI_DISP_CHECK_INVAILDDEV(DispDev);
    MI_DISP_CHECK_NULLPOINTER(pstVgaParam);
    //MI_DISP_DOWN(&(astDevStatus[DispDev].stDevMutex));
    MI_DISP_GetVarMutex();
...
{
    MI_S32 s32Ret = E_MI_ERR_FAILED;
    DISP_DBG_INFO("MI_DISP_GetHdmiParam !!!\n");
    MI_DISP_CHECK_INVAILDDEV(DispDev);
...
MI_S32 MI_DISP_IMPL_SetHdmiParam(MI_DISP_DEV DispDev, MI_DISP_HdmiParam_t *pstHdmiParam)
{
    ...
    stHalHdmiParams.stCsc.u32Contrast = (pstHdmiParam->stCsc.u32Contrast * 255 / 100);
    stHalHdmiParams.stCsc.u32Hue = pstHdmiParam->stCsc.u32Hue;
    stHalHdmiParams.stCsc.u32Luma = (pstHdmiParam->stCsc.u32Luma * 255 / 100);
    stHalHdmiParams.stCsc.u32Saturation = (pstHdmiParam->stCsc.u32Saturation * 255 / 100);
...
{
    MI_S32 s32Ret = E_MI_ERR_FAILED;
    DISP_DBG_INFO("MI_DISP_GetCvbsParam !!!\n");
    MI_DISP_CHECK_INVAILDDEV(DispDev);
...
MI_S32 MI_DISP_IMPL_SetCvbsParam(MI_DISP_DEV DispDev, MI_DISP_CvbsParam_t *pstCvbsParam)
{
    ...
    DISP_DBG_INFO("MI_DISP_SetCvbsParam !!!\n");
    MI_DISP_CHECK_INVAILDDEV(DispDev);
    MI_DISP_CHECK_NULLPOINTER(pstCvbsParam);
    //MI_DISP_DOWN(&(astDevStatus[DispDev].stDevMutex));
    MI_DISP_GetVarMutex();
...
MI_U64 mi_display_ResetPts(MI_U64 u64FiredTimeStamp, MI_U64 u64FramePts, MI_BOOL* bBefore, MI_U32 u32VsyncInterval)
{
    ...
    DISP_DBG_ERR("Expect Fired pts: %llu, Cur frame pts: %lld u64Diff: %lld, pstDevStatus->u3264VsyncInterval: %u expect before: %d!!!\n", u64FiredTimeStamp, u64FramePts, u64Diff, u32VsyncInterval, *bBefore);
...
int Disp_Dev_ISR_Thread(void *pdata)
{
    ...
    while(!kthread_should_stop())
    {
        ...
        wait_event_interruptible_timeout((pstDispDevParam->stWaitQueueHead), FALSE, msecs_to_jiffies(10));
#endif
                ...
                for(u8PortCount = 0; u8PortCount < MI_DISP_VIDEO_LAYER_INPUT_PORT_MAX; u8PortCount++)
                {
                    ...
                    if(pstTmpLayer->astPortStatus[u8PortCount].bEnable)
                    {
                            ...
                            && (pstBufInfo != NULL))
                        {
                            ...
                            {
#if (defined MI_SYS_PROC_FS_DEBUG) && (MI_DISP_PROCFS_DEBUG == 1)
                                _mi_disp_StatFrameIntervaltime(pstBufInfo, u8LayerID, u8PortCount);
#endif
                                ...
                                pstPortPendingBuf->pstInputBuffer = pstBufInfo;
                                list_add(&(pstPortPendingBuf->stPortPendingBufNode), &astLayerParams[u8LayerID].stPortPendingBufQueue[u8PortCount]);
...
MI_S32 mi_disp_OnDumpDevAttr(MI_SYS_DEBUG_HANDLE_t handle, MI_U32 u32DevId, void *pUsrData)
{
    ...
    {
        handle.OnPrintOut(handle,"%10d",eInterface[i]);
        handle.OnPrintOut(handle,"%15d",astDevStatus[u32DevId].eDeviceTiming[eInterface[i]]);
    }
...
MI_S32 mi_disp_OnDumpChannelAttr(MI_SYS_DEBUG_HANDLE_t handle, MI_U32 u32DevId, void *pUsrData)
{
    ...
    if(!list_empty(&pstDispDevParam->stBindedLayer))
    {
        ...
        handle.OnPrintOut(handle,"%10s%15s%15s%15s%15s%15s\n","LayerId","LayerStatus","BindedDevID","ePixFormat","Compress","LayerWidth");
        list_for_each_safe(pstPos, n, &pstDispDevParam->stBindedLayer)
        {
            ...
            if(pstTmpLayer->bLayerEnabled == TRUE)
            {
                ...
                handle.OnPrintOut(handle,"%15d\n",pstTmpLayer->stVideoLayerAttr.stVidLayerSize.u16Width);
            }
        }
        handle.OnPrintOut(handle,"%10s%15s%15s%15s%15s%15s\n","LayerId","LayerHeight","LayDispWidth","LayDispHeight","Toleration","customer_alloc");
        list_for_each_safe(pstPos, n, &pstDispDevParam->stBindedLayer)
        {
            pstTmpLayer = list_entry(pstPos, mi_disp_LayerStatus_t, stLayerNode);
            u8LayerId = pstTmpLayer->u8LayerID;
            handle.OnPrintOut(handle,"%10d",u8LayerId);
            handle.OnPrintOut(handle,"%15d",pstTmpLayer->stVideoLayerAttr.stVidLayerSize.u16Height);
...
MI_S32 mi_disp_OnDumpInputPortAttr(MI_SYS_DEBUG_HANDLE_t handle, MI_U32 u32DevId, void *pUsrData)
{
    ...
    MI_U8 u8PortBufCnt = 0;
    MI_SYS_MOD_DEV_t *pstModDev = (MI_SYS_MOD_DEV_t*)astDevStatus[u32DevId].hDevSysHandle;
    for(u8LayerId = 0; u8LayerId < MI_DISP_VIDEO_LAYER_MAX; u8LayerId++)
    {
        ...
        handle.OnPrintOut(handle,"%10s%15s%15s%15s%15s%15s\n","PortId","FlipByGe","PtsResetCnt","FramePtsBefore","PendingBufCnt","QueueBufCnt");
        for(u32InPortId = 0; u32InPortId < MI_DISP_VIDEO_LAYER_INPUT_PORT_MAX; u32InPortId++)
        {
            if(astLayerParams[u8LayerId].astPortStatus[u32InPortId].bEnable == TRUE)
            {
                ...
                handle.OnPrintOut(handle,"%15llu",astLayerParams[u8LayerId].astPortStatus[u32InPortId].u64PtsResetCnt);
                handle.OnPrintOut(handle,"%15d",astLayerParams[u8LayerId].astPortStatus[u32InPortId].bFramePtsBefore);
                if(!list_empty(&astLayerParams[u8LayerId].stPortPendingBufQueue[u32InPortId]))
                {
                    struct list_head *pstPendingBufPos = NULL;
                    struct list_head *m = NULL;
                    mi_disp_PortPendingBuf_t *pstTmpPendingBuf;
                    list_for_each_safe(pstPendingBufPos, m, &astLayerParams[u8LayerId].stPortPendingBufQueue[u32InPortId])
                    {
                        pstTmpPendingBuf = list_entry(pstPendingBufPos, mi_disp_PortPendingBuf_t, stPortPendingBufNode);
                        if(pstTmpPendingBuf != NULL)
                            u8PortBufCnt++;
                    }
                }
                handle.OnPrintOut(handle,"%15d",u8PortBufCnt);
                handle.OnPrintOut(handle,"%15d\n",pstModDev->astChannels[0].pastInputPorts[u32InPortId]->stBindInputBufQueue.queue_buf_count);
            }
        }
    }
    return MI_SUCCESS;
...
MI_S32 mi_disp_OnHelp(MI_SYS_DEBUG_HANDLE_t  handle, MI_U32  u32DevId, void *pUsrData)
{
    ...
    handle.OnPrintOut(handle,"PrintFramePts: echo checkframepts [ON/OFF] > /proc/mi_modules/mi_disp/mi_disp%d\n",u32DevId);
    handle.OnPrintOut(handle,"StatFrameRate: echo statframerate [ON/OFF] > /proc/mi_modules/mi_disp/mi_disp%d\n",u32DevId);
    return MI_SUCCESS;
}
...
void _mi_disp_StatFrameIntervaltime(MI_SYS_BufInfo_t *pstBufInfo, MI_U8 LayerID, MI_U8 InPortID)
{
    stDISPStatFrame_t *pstStatFrame = &gstDISPStatFrameInfo[LayerID][InPortID];
    stDISPCheckFramePts_t *pstCheckFramePts = &gstDISPCheckFramePts[LayerID][InPortID];
    ...
    if(pstStatFrame->bstat == TRUE)
    {
        MI_U64 u64IntervalTime = 0;
        struct timespec sttime;
        memset(&sttime, 0, sizeof(sttime));
        do_posix_clock_monotonic_gettime(&sttime);
        pstStatFrame->u64CurrentInputFrameTime = ((MI_U64)sttime.tv_sec) * 1000000ULL + (sttime.tv_nsec / 1000);

        if(pstStatFrame->u64LastInputFrameTime != 0)
        {
            u64IntervalTime = pstStatFrame->u64CurrentInputFrameTime - pstStatFrame->u64LastInputFrameTime;
            pstStatFrame->u32StatBufCnt++;

            //PRINTF_PROC("interval time %llu \n", u64IntervalTime);

            if(u64IntervalTime > pstStatFrame->u64InterValTimeMAX[2])
            {
                pstStatFrame->u64InterValTimeMAX[0] = pstStatFrame->u64InterValTimeMAX[1];
                pstStatFrame->u64InterValTimeMAX[1] = pstStatFrame->u64InterValTimeMAX[2];
                pstStatFrame->u64InterValTimeMAX[2] = u64IntervalTime;
            }
            else if(u64IntervalTime > pstStatFrame->u64InterValTimeMAX[1])
            {
                pstStatFrame->u64InterValTimeMAX[0] = pstStatFrame->u64InterValTimeMAX[1];
                pstStatFrame->u64InterValTimeMAX[1] = u64IntervalTime;
            }
            else if(u64IntervalTime > pstStatFrame->u64InterValTimeMAX[0])
            {
                pstStatFrame->u64InterValTimeMAX[0] = u64IntervalTime;
            }

            if(u64IntervalTime < pstStatFrame->u64InterValTimeMIN[2])
            {
                pstStatFrame->u64InterValTimeMIN[0] = pstStatFrame->u64InterValTimeMIN[1];
                pstStatFrame->u64InterValTimeMIN[1] = pstStatFrame->u64InterValTimeMIN[2];
                pstStatFrame->u64InterValTimeMIN[2] = u64IntervalTime;
            }
            else if(u64IntervalTime < pstStatFrame->u64InterValTimeMIN[1])
            {
                pstStatFrame->u64InterValTimeMIN[0] = pstStatFrame->u64InterValTimeMIN[1];
                pstStatFrame->u64InterValTimeMIN[1] = u64IntervalTime;
            }
            else if(u64IntervalTime < pstStatFrame->u64InterValTimeMIN[0])
            {
                pstStatFrame->u64InterValTimeMIN[0] = u64IntervalTime;
            }
        }
        pstStatFrame->u64InterValTimeSum += u64IntervalTime;
        pstStatFrame->u64LastInputFrameTime = pstStatFrame->u64CurrentInputFrameTime;
    }
}

void _mi_disp_StatFrameTime(unsigned long arg)
{
    MI_U64 u64AverageIntervalFrametime = 0;
    MI_U8 u8LayerID = 0;
    MI_U8 u8PortID = 0;
    stDISPStatFrame_t *pstStatFrame;

    PRINTF_PROC("%10s%15s%15s%15s%15s%15s%15s%15s%15s%15s\n","LayerID","PortID","FrameCnt_1s","Average time",
    ...
    for(u8LayerID = 0; u8LayerID < MI_DISP_LAYER_MAX; u8LayerID ++)
    {
        ...
        {
            pstStatFrame = &gstDISPStatFrameInfo[u8LayerID][u8PortID];
            if(pstStatFrame->bstat != TRUE)
                continue;
            u64AverageIntervalFrametime = div_u64(pstStatFrame->u64InterValTimeSum, pstStatFrame->u32StatBufCnt);
            if(pstStatFrame->bFirstSet == FALSE)
            {
                PRINTF_PROC("%10d%15d%15d%15llu%15llu%15llu%15llu%15llu%15llu%15llu\n",u8LayerID,u8PortID,pstStatFrame->u32StatBufCnt,
                             u64AverageIntervalFrametime,pstStatFrame->u64InterValTimeMAX[0],pstStatFrame->u64InterValTimeMAX[1],pstStatFrame->u64InterValTimeMAX[2],
                             pstStatFrame->u64InterValTimeMIN[0],pstStatFrame->u64InterValTimeMIN[1],pstStatFrame->u64InterValTimeMIN[2]);
            }
            else
                pstStatFrame->bFirstSet = FALSE;

            pstStatFrame->u32StatBufCnt = 0;
            pstStatFrame->u64InterValTimeSum = 0;
            pstStatFrame->u64InterValTimeMAX[2] = 0;
            pstStatFrame->u64InterValTimeMAX[1] = 0;
            pstStatFrame->u64InterValTimeMAX[0] = 0;
            pstStatFrame->u64InterValTimeMIN[2] = -1;
            pstStatFrame->u64InterValTimeMIN[1] = -1;
            pstStatFrame->u64InterValTimeMIN[0] = -1;
            pstStatFrame->u64CurrentInputFrameTime = 0;
            pstStatFrame->u64LastInputFrameTime = 0;
        }
    }
    mod_timer(&gstDISPStatFrameTimer.Timer,jiffies+HZ);//set 1s timeout
}

MI_S32 mi_disp_StatFrameIntervalTime(MI_SYS_DEBUG_HANDLE_t handle, MI_U32 u32DevId, MI_U8 argc, MI_U8 **argv, void *pUsrData)
{
    MI_S32 s32Ret = E_MI_ERR_FAILED;
    MI_U8 u8LayerID = 0;
    MI_U8 u8PortID = 0;
        ...
        for(u8LayerID = 0; u8LayerID < MI_DISP_LAYER_MAX ; u8LayerID++)
        {
            for(u8PortID = 0; u8PortID < MI_DISP_INPUTPORT_MAX; u8PortID++)
            {
                ...
                if(astLayerParams[u8LayerID].astPortStatus[u8PortID].bEnable != TRUE)
                    continue;
                if (strcmp(argv[1], "ON") == 0 && pstStatFrame->bstat == FALSE)
                {
                    pstStatFrame->bstat = TRUE;
                    pstStatFrame->u32StatBufCnt = 0;
                    pstStatFrame->u64InterValTimeSum = 0;
                    pstStatFrame->u64InterValTimeMAX[2] = 0;
                    pstStatFrame->u64InterValTimeMAX[1] = 0;
                    ...
                    pstStatFrame->u64CurrentInputFrameTime = 0;
                    pstStatFrame->u64LastInputFrameTime = 0;
                    pstStatFrame->bFirstSet = TRUE;
                    handle.OnPrintOut(handle,"LayerID %d PortID %d bstat %d \r\n", u8LayerID, u8PortID, pstStatFrame->bstat);
                    if(gstDISPStatFrameTimer.bCreate == FALSE)
                    {
                        init_timer(&gstDISPStatFrameTimer.Timer);
                        gstDISPStatFrameTimer.Timer.function = _mi_disp_StatFrameTime;
                        add_timer(&gstDISPStatFrameTimer.Timer);
                        mod_timer(&gstDISPStatFrameTimer.Timer,jiffies+HZ);//set 1s timeout
                        gstDISPStatFrameTimer.bCreate = TRUE;
                    }
                }
                else if (strcmp(argv[1], "OFF") == 0 && pstStatFrame->bstat == TRUE)
                {
                    pstStatFrame->bstat = FALSE;
                    handle.OnPrintOut(handle,"LayerID %d PortID %d bstat %d \r\n", u8LayerID, u8PortID, pstStatFrame->bstat);
                    if(gstDISPStatFrameTimer.bCreate == TRUE)
                    {
                        del_timer(&gstDISPStatFrameTimer.Timer);
                        gstDISPStatFrameTimer.bCreate = FALSE;
                    }
                }
            }
        }
...
MI_S32 mi_disp_CheckFramePts(MI_SYS_DEBUG_HANDLE_t handle, MI_U32 u32DevId, MI_U8 argc, MI_U8 **argv, void *pUsrData)
{
    MI_S32 s32Ret = E_MI_ERR_FAILED;
    MI_U8 u8LayerID = 0;
    MI_U8 u8PortID = 0;
    ...
    if(argc > 1)
    {
        for(u8LayerID = 0; u8LayerID < MI_DISP_LAYER_MAX ; u8LayerID++)
        {
            for(u8PortID = 0; u8PortID < MI_DISP_INPUTPORT_MAX; u8PortID++)
            {
                pstCheckFramePts = &gstDISPCheckFramePts[u8LayerID][u8PortID];
                if(astLayerParams[u8LayerID].astPortStatus[u8PortID].bEnable != TRUE)
                    continue;
                if(strcmp(argv[1], "ON") == 0 && pstCheckFramePts->bCheckFramePts == FALSE)
                {
                ...
                else if (strcmp(argv[1], "OFF") == 0 && pstCheckFramePts->bCheckFramePts == TRUE)
                {
...
MI_S32 MI_DISP_IMPL_Init(void)
{
    ...
    for(u32DevID = 0; u32DevID < MI_DISP_DEV_MAX; u32DevID++)
    {
        ...
        mi_sys_RegistCommand("checkframepts", 1, mi_disp_CheckFramePts, hDevSysHandle);
        mi_sys_RegistCommand("statframerate", 1, mi_disp_StatFrameIntervalTime, hDevSysHandle);
#endif
    ...
    if(pstDevice0Param->bSupportIrq == TRUE)
    {
        s32IrqRet = request_irq(pstDevice0Param->u32DevIrq, MI_DISP_DevISR, IRQF_SHARED | IRQF_ONESHOT, "mi_disp_isr", pstDevice0Param);
        if(0 != s32IrqRet)
        {
            DISP_DBG_ERR("request_irq failed. u32DevIrq = %u, s32IrqRet = %d.\n\n ", pstDevice0Param->u32DevIrq, s32IrqRet);
        }
    }
    if(pstDevice1Param->bSupportIrq == TRUE)
    {
        s32IrqRet = request_irq(pstDevice1Param->u32DevIrq, MI_DISP_DevISR, IRQF_SHARED | IRQF_ONESHOT, "mi_disp_isr", pstDevice1Param);
        if(0 != s32IrqRet)
        {
            DISP_DBG_ERR("request_irq failed. u32DevIrq = %u, s32IrqRet = %d.\n\n ", pstDevice1Param->u32DevIrq, s32IrqRet);
        }
    }
