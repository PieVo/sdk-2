    ...
    MI_SHADOW_CALLBACK_EVENT_e *peCallbackEvent)
{
    ...
    s32WaitRes = wait_event_interruptible_timeout(pstDevInfo->stWaitCallback,FALSE, msecs_to_jiffies(u32TimeOutMs));
    if (!s32WaitRes)
    {
        *peCallbackEvent = MI_SHADOW_CALLBACK_EVENT_TIMEOUT;
        return MI_SUCCESS;
    }
    ...
    MI_SYS_ChnPort_t *pstChnPeerPort)
{
    ...
    if (MI_SHADOW_CALLBACK_EVENT_EXIT != eCallbackEvent)
    {
        ...
        if (!wait_event_interruptible_timeout(pstDevInfo->stWaitCallbackResult,FALSE, msecs_to_jiffies(1000)))
        {
    ...
    MI_SYS_BUF_HANDLE *phBufHandle)
{
    ...
    if (!pstSysBufInfo)
    {
        DBG_ERR("Call Get Sys Output Port Buf Fail, hShadow:0x%x, u32ChnId:%d, u32PortId:%d\n",
            hShadow,
            u32ChnId,
            u32PortId);
        ...
        if (pstBufConfig)
        {
            DBG_ERR("Call Get Sys Output Port Buf Fail, eBufType:%d, u64TargetPts:%d\n",
    ...
    DOWN(&(_stDevMgr.mutex));
    *phBufHandle = idr_alloc(&(_stDevMgr.idrBufInfo), phMapToUserBuf, 0, 0x0FFFFFFF, GFP_KERNEL);
    if(*phBufHandle < 0)
    {
        UP(&(_stDevMgr.mutex));
