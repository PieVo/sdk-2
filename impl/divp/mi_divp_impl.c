...
#define MI_DIVP_OUTPUT_HEIGHT_MAX (2160)
...
static mi_divp_StatFrame_t gstStatFrameInfo[MI_DIVP_CHN_MAX_NUM];
static mi_divp_StatFrameTimer_t gstStatFrameTimer;
static MI_U16 u16ChnlCnt;

static mi_divp_ProcessOneChnl_t gstProcessOneChnl;
static MI_BOOL gbStopOneChnl[MI_DIVP_CHN_MAX_NUM];

void _mi_divp_IntervalTimeProcess(MI_DIVP_CHN DivpChn);
#endif
...
static void _mi_divp_TransVdecMfdecInfoToDivpHal(mi_vdec_DbInfo_t *pstDbInfo, MHAL_DIVP_MFdecInfo_t* pstHalMfdecInfo)
{
    ...
    pstHalMfdecInfo->u16StartX = pstDbInfo->u16StartX;
    pstHalMfdecInfo->u16StartY = pstDbInfo->u16StartY;
    pstHalMfdecInfo->u16Width= pstDbInfo->u16HSize;
        ...
        else
        {
            ...
            if(E_MHAL_DIVP_PIXEL_FORMAT_YUV422_YUYV == pstDivpInputInfo->ePxlFmt)
            {
                ...
                pstDivpInputInfo->u64BufAddr[0] = pstDivpInputInfo->u64BufAddr[0]
                                                                + stCapWin.u16Y * pstDivpInputInfo->u16InputWidth * 2 + stCapWin.u16X * 2;
            }
            else if(E_MHAL_DIVP_PIXEL_FORMAT_YUV_SEMIPLANAR_420 == pstDivpInputInfo->ePxlFmt)
            {
                ...
                pstDivpInputInfo->u64BufAddr[0] = pstDivpInputInfo->u64BufAddr[0]
                                                                + stCapWin.u16Y * pstDivpInputInfo->u16InputWidth + stCapWin.u16X;
                pstDivpInputInfo->u64BufAddr[1] = pstDivpInputInfo->u64BufAddr[1]
                                                                + stCapWin.u16Y / 2 * pstDivpInputInfo->u16InputWidth + stCapWin.u16X;
            }
                ...
                || (E_MHAL_DIVP_PIXEL_FORMAT_YC420_MSTTILE3_H265 == pstDivpInputInfo->ePxlFmt))
            {
                if(E_MHAL_DIVP_TILE_MODE_16x32 == pstDivpInputInfo->eTileMode)
                {
                    ...
                    pstDivpInputInfo->u64BufAddr[0] = pstDivpInputInfo->u64BufAddr[0]
                                                                    + stCapWin.u16Y * pstDivpInputInfo->u16InputWidth + stCapWin.u16X * 32;
                    pstDivpInputInfo->u64BufAddr[1] = pstDivpInputInfo->u64BufAddr[1]
                                                                    + stCapWin.u16Y / 2 * pstDivpInputInfo->u16InputWidth + stCapWin.u16X * 32;
                }
                else if(E_MHAL_DIVP_TILE_MODE_32x16 == pstDivpInputInfo->eTileMode)
                {
                    ...
                    pstDivpInputInfo->u64BufAddr[0] = pstDivpInputInfo->u64BufAddr[0]
                                                                    + stCapWin.u16Y * pstDivpInputInfo->u16InputWidth + stCapWin.u16X * 16;
                    pstDivpInputInfo->u64BufAddr[1] = pstDivpInputInfo->u64BufAddr[1]
                                                                    + stCapWin.u16Y / 2 * pstDivpInputInfo->u16InputWidth + stCapWin.u16X * 16;
                }
...
#if defined(MI_SYS_PROC_FS_DEBUG) &&(MI_DIVP_PROCFS_DEBUG == 1)
    _mi_divp_IntervalTimeProcess(pstTask->u32ChnId);
#endif
...
#if defined(MI_SYS_PROC_FS_DEBUG) &&(MI_DIVP_PROCFS_DEBUG == 1)
        {
            MI_DIVP_CHN DivpChn = pstTask->u32ChnId;
            if(gstStatFrameInfo[DivpChn].bCheckFrameID == TRUE
                &&gstStatFrameInfo[DivpChn].u16CheckFrameIDChanId == DivpChn)
            {
                gstStatFrameInfo[DivpChn].u32CheckFrameIDBufCnt ++;

                gstStatFrameInfo[DivpChn].u64Pts = pstTask->astInputPortBufInfo[DIVP_CHN_INPUT_PORT_ID]->u64Pts;
                PRINTF_PROC("ChnID %d, receive buffer ID = %u , PTS = %lld\n",
                gstStatFrameInfo[DivpChn].u16CheckFrameIDChanId, gstStatFrameInfo[DivpChn].u32CheckFrameIDBufCnt,
                gstStatFrameInfo[DivpChn].u64Pts);
            }
        }
#endif
    }
...
#if defined(MI_SYS_PROC_FS_DEBUG) &&(MI_DIVP_PROCFS_DEBUG == 1)
{
                ...
                &&gstStatFrameInfo[DivpChn].u16CheckFrameIDChanId == DivpChn)
            {
                ...
                gstStatFrameInfo[DivpChn].u64Pts = pstTask->astInputPortBufInfo[DIVP_CHN_INPUT_PORT_ID]->u64Pts;
                PRINTF_PROC("ChnID %d, receive buffer ID = %u , H = %u, L = %u, PTS = %lld\n",
                gstStatFrameInfo[DivpChn].u16CheckFrameIDChanId, gstStatFrameInfo[DivpChn].u32CheckFrameIDBufCnt,
    ...
    list_for_each_safe(pos, n, &divp_todo_common_task_list)
    {
        ...
        if(MI_SUCCESS == s32Ret)
        {
            *pu16Fence += 1;
            ...
            pstCmdInf->MHAL_CMDQ_CmdDelay(pstCmdInf, 100*1000);///100*1000ns = 100us = 0.1ms
...
MI_S32 mi_divp_ProcessCaptureCreenTask(MI_U16* pu16Fence, MHAL_CMDQ_CmdqInterface_t* pstCmdInf)
{
    ...
    list_for_each_safe(pos, n, &divp_todo_capture_task_list)
    {
        ...
        while(256 > pstCmdInf->MHAL_CMDQ_CheckBufAvailable(pstCmdInf, 256))//0x800 = 256 commands * 8 bytes/command
        {
            ...
            interruptible_sleep_on_timeout(&divp_isr_waitqueue, msecs_to_jiffies(2));
#elif LINUX_VERSION_CODE == KERNEL_VERSION(3,18,30)
            wait_event_interruptible_timeout(divp_isr_waitqueue, TRUE, msecs_to_jiffies(2));
#endif
#if 0
            u32LoopCnt++;
            if(u32LoopCnt>1000)
...
MI_S32 mi_divp_OnDumpInputPortAttr(MI_SYS_DEBUG_HANDLE_t handle,MI_U32 u32DevId,void *pUsrData)
{
    ...
    handle.OnPrintOut(handle,"%7s%10s%8s%7s%8s%7s%12s\n","ChnId","InputChg","bIPChg", "Width","Height","Pixel","GetBuffCnt");
    ...
    for(u32ChnId = 0; u32ChnId < (MI_DIVP_CHN_MAX_NUM +1); u32ChnId++)
    {
        if(gstDivpChnCtx[u32ChnId].eStatus > E_MI_DIVP_CHN_INITED)
        {
            ...
            handle.OnPrintOut(handle, "%12llu\n", gstDivpChnCtx[u32ChnId].u64GetBufferCnt);
...
MI_S32 mi_divp_OnDumpOutPortAttr(MI_SYS_DEBUG_HANDLE_t handle,MI_U32 u32DevId, void *pUsrData)
{
    ...
    handle.OnPrintOut(handle,"%7s%11s%7s%8s%7s%10s%10s%11s%10s%18s%17s\n","ChnId","OutputChg","Width","Height","Pixel","CompMode","PreWidth","PreHeight","PrePixel","PrepareOutBufCnt","FinishOutBufCnt");
    for(u32ChnId = 0; u32ChnId < (MI_DIVP_CHN_MAX_NUM +1); u32ChnId++)
    {
        if(gstDivpChnCtx[u32ChnId].eStatus > E_MI_DIVP_CHN_INITED)
        {
            ...
            handle.OnPrintOut(handle, "%18llu", gstDivpChnCtx[u32ChnId].u64PrepareOutBufCnt);
            handle.OnPrintOut(handle, "%17llu\n", gstDivpChnCtx[u32ChnId].u64FinishOutBufCnt);
...
void _mi_divp_IntervalTimeProcess(MI_DIVP_CHN DivpChn)
{
    if(gstStatFrameInfo[DivpChn].bstat == TRUE
        && gstStatFrameInfo[DivpChn].u16StatChanId == DivpChn)
    {
        MI_U64 u64IntervalTime = 0;
        struct timespec sttime;
        memset(&sttime, 0, sizeof(sttime));
        do_posix_clock_monotonic_gettime(&sttime);
        gstStatFrameInfo[DivpChn].u64CurrentInputFrameTime = ((MI_U64)sttime.tv_sec) * 1000000ULL + (sttime.tv_nsec / 1000);
        if(gstStatFrameInfo[DivpChn].u64LastInputFrameTime != 0)
        {
            u64IntervalTime = gstStatFrameInfo[DivpChn].u64CurrentInputFrameTime - gstStatFrameInfo[DivpChn].u64LastInputFrameTime;
            gstStatFrameInfo[DivpChn].u32StatBufCnt++;
            //PRINTF_PROC("interval time %llu \n", u64IntervalTime);

            if(u64IntervalTime > gstStatFrameInfo[DivpChn].u64InterValTimeMAX[2])
            {
                gstStatFrameInfo[DivpChn].u64InterValTimeMAX[0] = gstStatFrameInfo[DivpChn].u64InterValTimeMAX[1];
                gstStatFrameInfo[DivpChn].u64InterValTimeMAX[1] = gstStatFrameInfo[DivpChn].u64InterValTimeMAX[2];
                gstStatFrameInfo[DivpChn].u64InterValTimeMAX[2] = u64IntervalTime;
            }
            else if(u64IntervalTime > gstStatFrameInfo[DivpChn].u64InterValTimeMAX[1])
            {
                gstStatFrameInfo[DivpChn].u64InterValTimeMAX[0] = gstStatFrameInfo[DivpChn].u64InterValTimeMAX[1];
                gstStatFrameInfo[DivpChn].u64InterValTimeMAX[1] = u64IntervalTime;
            }
            else if(u64IntervalTime > gstStatFrameInfo[DivpChn].u64InterValTimeMAX[0])
            {
                gstStatFrameInfo[DivpChn].u64InterValTimeMAX[0] = u64IntervalTime;
            }

            if(u64IntervalTime < gstStatFrameInfo[DivpChn].u64InterValTimeMIN[2])
            {
                gstStatFrameInfo[DivpChn].u64InterValTimeMIN[0] = gstStatFrameInfo[DivpChn].u64InterValTimeMIN[1];
                gstStatFrameInfo[DivpChn].u64InterValTimeMIN[1] = gstStatFrameInfo[DivpChn].u64InterValTimeMIN[2];
                gstStatFrameInfo[DivpChn].u64InterValTimeMIN[2] = u64IntervalTime;
            }
            else if(u64IntervalTime < gstStatFrameInfo[DivpChn].u64InterValTimeMIN[1])
            {
                gstStatFrameInfo[DivpChn].u64InterValTimeMIN[0] = gstStatFrameInfo[DivpChn].u64InterValTimeMIN[1];
                gstStatFrameInfo[DivpChn].u64InterValTimeMIN[1] = u64IntervalTime;
            }
            else if(u64IntervalTime < gstStatFrameInfo[DivpChn].u64InterValTimeMIN[0])
            {
                gstStatFrameInfo[DivpChn].u64InterValTimeMIN[0] =u64IntervalTime;
            }
        }

        gstStatFrameInfo[DivpChn].u64InterValTimeSum += u64IntervalTime;
        gstStatFrameInfo[DivpChn].u64LastInputFrameTime = gstStatFrameInfo[DivpChn].u64CurrentInputFrameTime;
    }
}

void _mi_divp_StatFrameTime(unsigned long arg)
{
    MI_U64 u64AverageIntervalFrametime = 0;
    MI_DIVP_CHN DivpChn = 0;
    for(DivpChn =0; DivpChn < MI_DIVP_CHN_MAX_NUM; DivpChn ++)
    {
        if(gstStatFrameInfo[DivpChn].bstat == TRUE && gstStatFrameInfo[DivpChn].u32StatBufCnt != 0)
        {
            u64AverageIntervalFrametime = div_u64(gstStatFrameInfo[DivpChn].u64InterValTimeSum, gstStatFrameInfo[DivpChn].u32StatBufCnt);

            if(gstStatFrameInfo[DivpChn].bFirstSet == FALSE)
            {
                PRINTF_PROC("ChnlID %d,1s FrameCnt:%d,Average time: %-6lluus,MAX %-6lluus,%-6lluus,%-6lluus,MIN %-6lluus,%-6lluus,%-6lluus\n",
                    gstStatFrameInfo[DivpChn].u16StatChanId, gstStatFrameInfo[DivpChn].u32StatBufCnt, u64AverageIntervalFrametime,
                    gstStatFrameInfo[DivpChn].u64InterValTimeMAX[2], gstStatFrameInfo[DivpChn].u64InterValTimeMAX[1], gstStatFrameInfo[DivpChn].u64InterValTimeMAX[0],
                    gstStatFrameInfo[DivpChn].u64InterValTimeMIN[2], gstStatFrameInfo[DivpChn].u64InterValTimeMIN[1], gstStatFrameInfo[DivpChn].u64InterValTimeMIN[0]);
            }
            else
                gstStatFrameInfo[DivpChn].bFirstSet = FALSE;

            gstStatFrameInfo[DivpChn].u32StatBufCnt = 0;
        }

        gstStatFrameInfo[DivpChn].u64InterValTimeSum = 0;
        gstStatFrameInfo[DivpChn].u64InterValTimeMAX[2] = 0;
        gstStatFrameInfo[DivpChn].u64InterValTimeMAX[1] = 0;
        gstStatFrameInfo[DivpChn].u64InterValTimeMAX[0] = 0;
        gstStatFrameInfo[DivpChn].u64InterValTimeMIN[2] = -1;
        gstStatFrameInfo[DivpChn].u64InterValTimeMIN[1] = -1;
        gstStatFrameInfo[DivpChn].u64InterValTimeMIN[0] = -1;
        gstStatFrameInfo[DivpChn].u64CurrentInputFrameTime = 0;
        gstStatFrameInfo[DivpChn].u64LastInputFrameTime = 0;

    mod_timer(&gstStatFrameTimer.Timer,jiffies+HZ);//set 1s timeout
    }
}

MI_S32 mi_divp_StatFrameIntervalTime(MI_SYS_DEBUG_HANDLE_t handle, MI_U32 u32DevId, MI_U8 argc, MI_U8 **argv, void *pUsrData)
{
    ...
    if (argc > 2)
    {
        ...
        if(gstStatFrameInfo[DivpChn].bEnable == TRUE)
        {
            gstStatFrameInfo[DivpChn].u16StatChanId = DivpChn;
        }
        else
        ...
        if (strcmp(argv[2], "ON") == 0 && gstStatFrameInfo[DivpChn].bstat == FALSE)
        {
            gstStatFrameInfo[DivpChn].bstat = TRUE;
            ...
            gstStatFrameInfo[DivpChn].u64CurrentInputFrameTime = 0;
            gstStatFrameInfo[DivpChn].u64LastInputFrameTime = 0;
            gstStatFrameInfo[DivpChn].bFirstSet = TRUE;

            u16ChnlCnt ++;
        }
        else if (strcmp(argv[2], "OFF") == 0 && gstStatFrameInfo[DivpChn].bstat == TRUE)
        {
            gstStatFrameInfo[DivpChn].bstat = FALSE;
            u16ChnlCnt --;
        }
        else
        {
        ...
        if(u16ChnlCnt >0 && gstStatFrameTimer.bCreate == FALSE)
        {
            init_timer(&gstStatFrameTimer.Timer);
            gstStatFrameTimer.Timer.function=_mi_divp_StatFrameTime;
            add_timer(&gstStatFrameTimer.Timer);
            mod_timer(&gstStatFrameTimer.Timer,jiffies+HZ);//set 1s timeout

            gstStatFrameTimer.bCreate = TRUE;
        }

        if(u16ChnlCnt == 0 && gstStatFrameTimer.bCreate == TRUE)
        {
            del_timer(&gstStatFrameTimer.Timer);
            gstStatFrameTimer.bCreate = FALSE;
        }

        handle.OnPrintOut(handle,"statChanID %d bstat %d \r\n", gstStatFrameInfo[DivpChn].u16StatChanId, gstStatFrameInfo[DivpChn].bstat);
        s32Ret = MI_SUCCESS;
    }
    else
    {
        handle.OnPrintOut(handle, "Unsupport command count: %d.\n", argc);
        handle.OnPrintOut(handle, "DIVP STAT FRAME Intervaltime CHANNEL ID; [ON, OFF];\n");
...
MI_S32 mi_divp_CheckFrameID(MI_SYS_DEBUG_HANDLE_t handle, MI_U32 u32DevId, MI_U8 argc, MI_U8 **argv, void *pUsrData)
{
    ...
    if (argc > 2)
    {
        ...
        if (strcmp(argv[2], "ON") == 0 && gstStatFrameInfo[DivpChn].bCheckFrameID == FALSE)
        {
            gstStatFrameInfo[DivpChn].u32CheckFrameIDBufCnt = 0;
           gstStatFrameInfo[DivpChn].bCheckFrameID = TRUE;
        }
        else if (strcmp(argv[2], "OFF") == 0 && gstStatFrameInfo[DivpChn].bCheckFrameID == TRUE)
        {
            gstStatFrameInfo[DivpChn].bCheckFrameID = FALSE;
        }
        else
        {
            handle.OnPrintOut(handle, "argv[2] is error[ON, OFF].\n", DivpChn);
        ...
        handle.OnPrintOut(handle,"ChanID %d bCheckFrameID %d \r\n",gstStatFrameInfo[DivpChn].u16CheckFrameIDChanId, gstStatFrameInfo[DivpChn].bCheckFrameID);
        s32Ret = MI_SUCCESS;
    }
    else
    {
...
MI_S32 mi_divp_Init(void)
{
    ...
    mi_sys_RegistCommand("statintervaltime", 2, mi_divp_StatFrameIntervalTime, gDivpDevHdl);
...
#if defined(MI_SYS_PROC_FS_DEBUG) &&(MI_DIVP_PROCFS_DEBUG == 1)
    if(gstStatFrameTimer.bCreate == TRUE)
    {
        del_timer(&gstStatFrameTimer.Timer);
        gstStatFrameTimer.bCreate = FALSE;
    }
    ...
    else
    {
        ...
        if(MI_SUCCESS == s32Ret)
        {
                                            ...
                                            mi_divp_Alloc, mi_divp_Free, DivpChn, &gstDivpChnCtx[DivpChn].pHalCtx))
            {
                ...
                gu32ChnNum ++;
                #if defined(MI_SYS_PROC_FS_DEBUG) &&(MI_DIVP_PROCFS_DEBUG == 1)
                gstStatFrameInfo[DivpChn].bEnable = TRUE;
                #endif
                MI_DIVP_UNLOCK_CHN_GLOBAL_VARIABLE(DivpChn);
            }
            else
        ...
        {
            DBG_ERR(" Fail to create channel. invalid input parameter.\n");
        }
    }
...
MI_S32 MI_IMPL_DIVP_DestroyChn(MI_DIVP_CHN DivpChn)
{
    ...
    {
        DBG_ERR(" destroy channel fail. channel is not stoped. u32ChnId = %d.\n", DivpChn);
    ...
    else
    {
        ...
        if((MI_SUCCESS == s32Ret) && (MHAL_SUCCESS == MHAL_DIVP_DestroyInstance(gstDivpChnCtx[DivpChn].pHalCtx)))
        {
            ...
            #if defined(MI_SYS_PROC_FS_DEBUG) &&(MI_DIVP_PROCFS_DEBUG == 1)
            gstStatFrameInfo[DivpChn].bEnable = FALSE;
            #endif
            gu32ChnNum --;
        }
        else
        {
            DBG_ERR(" Fail to destroy channel. s32Ret = %d \n", s32Ret);
            s32Ret = MI_DIVP_ERR_FAILED;
        }
...
MI_S32 MI_IMPL_DIVP_SetChnAttr(MI_DIVP_CHN DivpChn, MI_DIVP_ChnAttr_t* pstAttr)
{
    ...
    {
        DBG_ERR(" invalid channel attribute!!\n");
        s32Ret = MI_DIVP_ERR_INVALID_PARAM;
    }
...
MI_S32 MI_IMPL_DIVP_StartChn(MI_DIVP_CHN DivpChn)
{
    ...
    mi_sys_EnableOutputPort(gDivpDevHdl, DivpChn, DIVP_CHN_OUTPUT_PORT_ID);
    mi_sys_EnableInputPort(gDivpDevHdl, DivpChn, DIVP_CHN_INPUT_PORT_ID);
    mi_sys_EnableChannel(gDivpDevHdl, DivpChn);
...
MI_S32 MI_IMPL_DIVP_SetOutputPortAttr(MI_DIVP_CHN DivpChn, MI_DIVP_OutputPortAttr_t * pstOutputPortAttr)
{
    ...
    DBG_INFO("DivpChn = %d. port Width = %u, Height = %u, pixelformat = %u, eCompressMode = %d.\n", DivpChn, pstOutputPortAttr->u32Width,
    ...
    {
        DBG_ERR(" invalid output port attribute!!\n");
        s32Ret = MI_DIVP_ERR_INVALID_PARAM;
    }
    else
    {
