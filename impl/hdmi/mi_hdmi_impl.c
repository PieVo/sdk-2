...
static MI_S32 _MI_HDMI_IMPL_GetCapability(MI_SYS_DEBUG_HANDLE_t handle, MI_U32 u32DevId, MI_U8 argc, MI_U8 **argv, void *pUsrData)
{
    ...
    {
        handle.OnPrintOut(handle, "%10s", "Y");
    }
    else
    {
        handle.OnPrintOut(handle, "%10s", "N");
    }
    if (_stHdmiMgr.bAvMute)
    {
        handle.OnPrintOut(handle, "%8s", "Y");
    }
    else
    {
        handle.OnPrintOut(handle, "%8s", "N");
    }
    ...
    handle.OnPrintOut(handle, "%13d", _stHdmiMgr.stAttr.stVideoAttr.bEnableVideo);
    handle.OnPrintOut(handle, "%12d", _stHdmiMgr.stInerAttr.eHdmiVideoTiming);
    handle.OnPrintOut(handle, "%12d", _stHdmiMgr.stInerAttr.eHdmiTxMode);
    handle.OnPrintOut(handle, "%11d", _stHdmiMgr.stAttr.stVideoAttr.eColorType);
    handle.OnPrintOut(handle, "%9d",  _stHdmiMgr.stInerAttr.eHdmiTxInColor);
    handle.OnPrintOut(handle, "%10d", _stHdmiMgr.stInerAttr.eHdmiTxOutColor);
    handle.OnPrintOut(handle, "%15d\n", _stHdmiMgr.stAttr.stVideoAttr.eDeepColorMode);

    //for audio
    handle.OnPrintOut(handle, "\n%13s%16s%10s%10s%13s\n", "EnableAudio", "IsMultiChannel", "BitDepth", "CodeType", "SampleRate\n");
    handle.OnPrintOut(handle, "%13d",_stHdmiMgr.stAttr.stAudioAttr.bEnableAudio);
    handle.OnPrintOut(handle, "%16d",_stHdmiMgr.stAttr.stAudioAttr.bIsMultiChannel);
    handle.OnPrintOut(handle, "%10d",_stHdmiMgr.stAttr.stAudioAttr.eBitDepth);
    handle.OnPrintOut(handle, "%10d",_stHdmiMgr.stAttr.stAudioAttr.eCodeType);
    handle.OnPrintOut(handle, "%12d\n",_stHdmiMgr.stAttr.stAudioAttr.eSampleRate);
...
MI_S32 MI_HDMI_IMPL_Init()
{
    ...
    _stHdmiMgr.stAttr.stVideoAttr.eDeepColorMode = E_MI_HDMI_DEEP_COLOR_MAX;
