        ...
        do
        {
            ...
            if(MI_SUCCESS  == MI_SYS_ChnInputPortGetBuf(&stVpeChnInputPort0, &stBufConf, &stBufInfo, &hHandle , 0))
            {
               // Start user put int buffer
                ...
                {
                    MI_SYS_FlushCache(stBufInfo.stFrameData.pVirAddr[0], stBufInfo.stFrameData.u32Stride[0]*u16Height);
                    MI_SYS_FlushCache(stBufInfo.stFrameData.pVirAddr[1], stBufInfo.stFrameData.u32Stride[1]*u16Height/2);
