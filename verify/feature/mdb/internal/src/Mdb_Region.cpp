...
static const char * _gszFileName[MDB_RGN_BMP_MAXNUM] = {"/mnt/1555/64X48.argb1555", "/mnt/1555/80X80.argb1555", "/mnt/1555/200X200.argb1555", "/mnt/1555/304X200.argb1555",
                                                        "/mnt/4444/64X48.argb4444", "/mnt/4444/80X80.argb4444", "/mnt/4444/200X200.argb4444", "/mnt/4444/304X200.argb4444",
...
static const MI_RGN_Size_t _gstSize[MDB_RGN_BMP_MAXNUM] = {{64, 48}, {80, 80}, {200, 200}, {304, 200},
...
void Mdb_Region::Init(std::vector<std::string> &inStrings, std::string &strOut)
{
    ...
    if (!inStrings.empty())
    {
        ...
        MDB_EXPECT_OK("MI_RGN_Init", strOut, MI_RGN_Init(&stPaletteTable));

    }
    else
    {
        MDB_EXPECT_OK("MI_RGN_Init", strOut, MI_RGN_Init(&_gstPaletteTable));
    }
}
void Mdb_Region::Deinit(std::vector<std::string> &inStrings, std::string &strOut)
{
    MDB_EXPECT_OK("MI_RGN_Deinit", strOut, MI_RGN_DeInit());
}
void Mdb_Region::Create(std::vector<std::string> &inStrings, std::string &strOut)
{
    ...
    MDB_EXPECT_OK("MI_RGN_Create", strOut, MI_RGN_Create(hHandle, &stRegion));
}
void Mdb_Region::Destroy(std::vector<std::string> &inStrings, std::string &strOut)
{
    ...
    MDB_EXPECT_OK("MI_RGN_Destroy", strOut, MI_RGN_Destroy(hHandle));
}
void Mdb_Region::Attach(std::vector<std::string> &inStrings, std::string &strOut)
{
    ...
    stChnAttr.stCoverPara.u32Color = (MI_U32)Atoi(inStrings[11]);

    MDB_EXPECT_OK("MI_RGN_AttachToChn", strOut, MI_RGN_AttachToChn(hHandle, &stChnPort, &stChnAttr));
}
void Mdb_Region::Dettach(std::vector<std::string> &inStrings, std::string &strOut)
{
    ...
    MDB_EXPECT_OK("MI_RGN_DetachFromChn", strOut, MI_RGN_DetachFromChn(hHandle, &stChnPort));
}
...
void Mdb_Region::GetAttr(std::vector<std::string> &inStrings, std::string &strOut)
{
    ...
    MDB_EXPECT_OK("MI_RGN_GetAttr", strOut, MI_RGN_GetAttr(hHandle, &stRegion));
}
...
void Mdb_Region::SetDisplayAttr(std::vector<std::string> &inStrings, std::string &strOut)
{
    ...
    stChnPortAttr.stCoverPara.u32Color = (MI_U32)Atoi(inStrings[11]);

    MDB_EXPECT_OK("MI_RGN_SetDisplayAttr", strOut, MI_RGN_SetDisplayAttr(hHandle, &stChnPort, &stChnPortAttr));
}
...
void Mdb_Region::SetBitMap(std::vector<std::string> &inStrings, std::string &strOut)
{
    ...
    MDB_EXPECT_OK("MI_RGN_SetBitMap", strOut, MI_RGN_SetBitMap(hHandle, &stBitmap));
    free(pu8FileBuffer);
}
...
void Mdb_Region::GetCanvas(std::vector<std::string> &inStrings, std::string &strOut)
{
    ...
    MDB_EXPECT_OK("MI_RGN_GetCanvasInfo", strOut, MI_RGN_GetCanvasInfo(hHandle, &stCanvasInfo));
}
void Mdb_Region::UpdateCanvas(std::vector<std::string> &inStrings, std::string &strOut)
{
    ...
    MDB_EXPECT_OK("MI_RGN_UpdateCanvas", strOut, MI_RGN_UpdateCanvas(hHandle));
}

// create 1 -> destroy 1
void Mdb_Region::TestCase001(std::vector<std::string> &inStrings, std::string &strOut)
{
    MDB_EXPECT_OK_ERRCASE("MI_RGN_Create", strOut, MI_RGN_Create(_ghHandle, &_gstRgnAttr), {});
    MDB_EXPECT_OK_ERRCASE("MI_RGN_Destroy", strOut, MI_RGN_Destroy(_ghHandle), {});
...
void Mdb_Region::TestCase003(std::vector<std::string> &inStrings, std::string &strOut)
{
    ...
    MDB_EXPECT_OK_ERRCASE(ss.str(), strOut, MI_RGN_Create(_ghHandle, &_gstRgnAttr),
            {SetDefaultParam();ClearRegion(inStrings, strOut);return;};);
    ss.str("");
    ss << "MI_RGN_AttachToChn" << "(" << strRgnType << " " << strDettachInfo <<")";
    MDB_EXPECT_OK_ERRCASE(ss.str(), strOut, MI_RGN_AttachToChn(_ghHandle, &_gstChnPort, &_gstChnPortParam),
            {SetDefaultParam();ClearRegion(inStrings, strOut);return;};);
    if (!bNotDettach)
    {
        ss.str("");
        ss << "MI_RGN_DetachFromChn" << "(" << strRgnType << " " << strDettachInfo <<")";
        MDB_EXPECT_OK_ERRCASE(ss.str(), strOut, MI_RGN_DetachFromChn(_ghHandle, &_gstChnPort),
                {SetDefaultParam();ClearRegion(inStrings, strOut);return;});
    }
    ss.str("");
    ss << "MI_RGN_Destroy" << "(" << strRgnType << " " << strDettachInfo <<")";
    MDB_EXPECT_OK_ERRCASE(ss.str(), strOut, MI_RGN_Destroy(_ghHandle),
            {SetDefaultParam();ClearRegion(inStrings, strOut);return;};);
...
void Mdb_Region::TestCase005(std::vector<std::string> &inStrings, std::string &strOut)
{
    ...
    for (MI_U8 i = 0; i < u8RgnMaxNum+1; i++)
    {
        ...
        if (u8RgnMaxNum == i)
        {
            MDB_EXPECT_FAIL_ERRCASE(ss.str(), strOut, MI_RGN_AttachToChn(_ghHandle, &_gstChnPort, &_gstChnPortParam), SetDefaultParam();ClearRegion(inStrings, strOut);return;);
            break;
        }
        else
        {
            MDB_EXPECT_OK_ERRCASE(ss.str(), strOut, MI_RGN_AttachToChn(_ghHandle, &_gstChnPort, &_gstChnPortParam), SetDefaultParam();ClearRegion(inStrings, strOut);return;);
        }
...
void Mdb_Region::TestCase007(std::vector<std::string> &inStrings, std::string &strOut)
{
    ...
    switch(u8FlowId)
    {
        case 0:
            ...
            MDB_EXPECT_OK_ERRCASE("MI_RGN_Create", strOut, MI_RGN_Create(_ghHandle, &stRgnAttr),
                {SetDefaultParam();ClearRegion(inStrings, strOut);return;});
            MDB_EXPECT_OK_ERRCASE("MI_RGN_GetCanvasInfo", strOut, MI_RGN_GetCanvasInfo(_ghHandle, &stCanvasInfo),
                {SetDefaultParam();ClearRegion(inStrings, strOut);return;});
            ...
            MDB_EXPECT_OK_ERRCASE("MI_RGN_UpdateCanvas", strOut, MI_RGN_UpdateCanvas(_ghHandle),
                {SetDefaultParam();ClearRegion(inStrings, strOut);return;});
            MDB_EXPECT_OK_ERRCASE("MI_RGN_AttachToChn", strOut, MI_RGN_AttachToChn(_ghHandle, &_gstChnPort, &_gstChnPortParam),
                {SetDefaultParam();ClearRegion(inStrings, strOut);return;});
            vecStrIn.clear();
            memset(szParamIn, 0, sizeof(szParamIn));
            sprintf(szParamIn, "%d %d", (MI_U8)_gstChnPort.eModId, (MI_U8)_gstChnPort.s32ChnId);
...
void Mdb_Region::TestCase009(std::vector<std::string> &inStrings, std::string &strOut)
{
    ...
    while(_ghHandle > 0)
    {
        ...
        MDB_EXPECT_OK_ERRCASE(ss.str(), strOut, MI_RGN_Destroy(_ghHandle-1),
        {SetDefaultParam();ClearRegion(inStrings, strOut);return;});
        _ghHandle--;
    }
...
void Mdb_Region::OneRegionOneAttach(std::vector<std::string> &inStrings, std::string &strOut)
{
    ...
    stChnAttr.stCoverPara.u32Color = 0x0333;
    stChnAttr.stCoverPara.u32Layer = 1;

    MDB_EXPECT_OK_ERRCASE("MI_RGN_Create", strOut, MI_RGN_Create(hHandle, &stRgnAttr), return;);
    MDB_EXPECT_OK_ERRCASE("MI_RGN_AttachToChn", strOut, MI_RGN_AttachToChn(hHandle, &stChnPort, &stChnAttr), return;);
...
void Mdb_Region::OneChnOnePortSetBitmap(std::vector<std::string> &inStrings, std::string &strOut)
{
    ...
    for (i = 0; i < MDB_RGN_OSD_MAX_NUM-1; i++)
    {
        ...
        MDB_EXPECT_OK_ERRCASE(ss.str(), strOut, MI_RGN_DetachFromChn(_ghHandle-1-i, &_gstChnPort),
            {SetDefaultParam();ClearRegion(inStrings, strOut);return;});
        vecStrIn.clear();
        memset(szParamIn, 0, sizeof(szParamIn));
        sprintf(szParamIn, "%d %d", (MI_U8)_gstChnPort.eModId, (MI_U8)_gstChnPort.s32ChnId);
...
void Mdb_Region::CreateAttachDetechDestroy(std::vector<std::string> &inStrings, std::string &strOut)
{
    ...
    MDB_EXPECT_OK_ERRCASE("MI_RGN_Create", strOut, MI_RGN_Create(hHandle, &stRgnAttr), return);
    MDB_EXPECT_OK_ERRCASE("MI_RGN_AttachToChn", strOut, MI_RGN_AttachToChn(hHandle, &stChnPort, &stChnPortParam), return);
    MDB_EXPECT_OK_ERRCASE("MI_RGN_DetachFromChn", strOut, MI_RGN_DetachFromChn(hHandle, &stChnPort), return);
    MDB_EXPECT_OK_ERRCASE("MI_RGN_Destroy", strOut, MI_RGN_Destroy(hHandle), return);
}
...
void Mdb_Region::VpeDivpAddData(std::vector<std::string> &inStrings, std::string &strOut)
{
    ...
    ss << "0 0 " <<intFormat<<" 64 48";
    ParseStrings(ss.str().c_str(), vecStrIn);
    Create(vecStrIn, strOut);
    vecStrIn.clear();
    ss.str("");

    ss << "1 0 " <<intFormat<<" 80 80";
    ParseStrings(ss.str().c_str(), vecStrIn);
    Create(vecStrIn, strOut);
    vecStrIn.clear();
    ss.str("");

    ss << "2 0 " <<intFormat<<" 200 200";
    ParseStrings(ss.str().c_str(), vecStrIn);
    Create(vecStrIn, strOut);
    vecStrIn.clear();
    ss.str("");

    ss << "3 0 " <<intFormat<<" 304 200";
    ParseStrings(ss.str().c_str(), vecStrIn);
    Create(vecStrIn, strOut);
    vecStrIn.clear();
    ss.str("");
...
void Mdb_Region::VpeDivpAttachData(std::vector<std::string> &inStrings, std::string &strOut)
{
    ...
    for (i = 0; i < intMaxChn; i++)
    {
        for (j = 0; j < 4; j++)
        {
            if (!intPort[j])
                continue;
            ss << "0 " << intMod << " 0 "<< i << " " << j << " 1 100 200 0 0 0 0";
            ParseStrings(ss.str().c_str(), vecStrIn);
            Attach(vecStrIn, strOut);
            vecStrIn.clear();
            ss.str("");

            ss << "1 " << intMod << " 0 " << i << " " << j << " 1 64 100 0 0 0 0";
            ParseStrings(ss.str().c_str(), vecStrIn);
            Attach(vecStrIn, strOut);
            vecStrIn.clear();
            ss.str("");

            ss << "2 " << intMod << " 0 " << i << " " << j << " 1 200 200 0 0 0 0";
            ParseStrings(ss.str().c_str(), vecStrIn);
            Attach(vecStrIn, strOut);
            vecStrIn.clear();
            ss.str("");

            ss << "3 " << intMod << " 0 " << i << " " << j << " 1 400 400 0 0 0 0";
            ParseStrings(ss.str().c_str(), vecStrIn);
            Attach(vecStrIn, strOut);
            vecStrIn.clear();
            ss.str("");
