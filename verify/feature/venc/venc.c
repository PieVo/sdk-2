...
int _VENC_FillFrameData(MI_SYS_BufInfo_t *pstBufInfo, VENC_InputType_e eInputType, MI_U32 u32Frame)
{
    ...
    case E_VENC_INPUT_FROM_CODE:
        ...
        MI_SYS_FlushCache(pstBufInfo->stFrameData.pVirAddr[0], size);
        ret = (int)size;
        break;
    case E_VENC_INPUT_FROM_HEADER:
        ...
        MI_SYS_FlushCache(pstBufInfo->stFrameData.pVirAddr[0], u32YSize);
        MI_SYS_FlushCache(pstBufInfo->stFrameData.pVirAddr[1], u32YSize >> 1);
        ret = u32FrameSize;
        break;
    ...
    while (1)
    {
        ...
        for (VencChn = 0; VencChn < s32CreateChnNum; ++VencChn)
        {
            ...
            if(MI_SUCCESS == s32Ret)
            {
                ...
                ret = _VENC_FillFrameData(&stBufInfo, eInputType, u32FrameSet);

                //print debug messages
                if (u32FrameSet == 0 && s32InMsgLvl > 0)
                {
...
int _VENC_ConfigH265(int iChn)
{
    ...
    ExecFunc(bErr, FALSE);
    {
        ...
        //s32Ret = MI_VENC_SetH264Trans(iChn, &stTrans);
        //DBG_ERR("s32Ret %X\n", s32Ret);
...
int _VENC_SetParam(int iChn, MI_VENC_ModType_e eModType)
{
    ...
    if(eModType == E_MI_VENC_MODTYPE_H265E)
    {
        return _VENC_SetH265Param(iChn);
    }
    if(eModType == E_MI_VENC_MODTYPE_H264E)
    {
        return _VENC_SetH264Param(iChn);
    }
...
int main(int argc, const char *argv[])
{
    ...
    for (VencChn = 0; VencChn < s32CreateChnNum; ++VencChn)
    {
        ...
        ExecFunc(_VENC_SetParam(VencChn, stChnAttr.stVeAttr.eType), MI_SUCCESS);
    }
