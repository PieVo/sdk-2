...
MS_BOOL MHAL_DISP_VideoLayerSetAttr(void *pVidLayerCtx, const MHAL_DISP_VideoLayerAttr_t *pstAttr)
{
    ...
    if(pstLayerIns->bEnable != TRUE)
    {
        ...
        if(pstAttr->ePixFormat == E_MHAL_DISP_PIXEL_FRAME_YC420_MSTTILE1_H264)
        {
            MDrv_MVOP_SetTileFormat(E_MVOP_TILE_16x32); //for h264
        }
        else if(pstAttr->ePixFormat == E_MHAL_DISP_PIXEL_FRAME_YC420_MSTTILE2_H265)
        {
            MDrv_MVOP_SetTileFormat(E_MVOP_TILE_32x16); //for h265
        }
        //Set Mvop output Config
        MDrv_MVOP_SetCommand(&stMvopHd, E_MVOP_CMD_SET_HANDSHAKE_MODE, &bHS);
        if(pstAttr->stVidLayerSize.u32Width == 2560 && pstAttr->stVidLayerSize.u32Height == 1440)
        {
            bXCGen = FALSE;
...
MS_BOOL MHAL_DISP_DeviceSetHdmiParam(void *pDevCtx, const MHAL_DISP_HdmiParam_t *pstHdmiInfo)
{
    ...
    stPQLColorTemperature.u32Contrast = pstHdmiInfo->stCsc.u32Contrast;
    stPQLColorTemperature.u32Hue      = pstHdmiInfo->stCsc.u32Hue;
    stPQLColorTemperature.u32Brightness  = pstHdmiInfo->stCsc.u32Luma;
    stPQLColorTemperature.u32Saturation  = pstHdmiInfo->stCsc.u32Saturation;
