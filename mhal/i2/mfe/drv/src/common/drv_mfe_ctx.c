    ...
    int     on)
{
    ...
    do
    {
        ...
        {
            mhve_cfg mcfg;
            int size_out, size_mbs, size_mbp, size_dqm, size_refrec;
    ...
    if (mctx->i_state == MMFE_CTX_STATE_NULL)
    {
        ...
        switch (parm->type)
        {
            case MMFE_PARM_RES:
                ...
                mctx->i_picture_w = _ALIGN_(4, parm->res.i_pict_w);     // input image
                mctx->i_picture_h = _ALIGN_(4, parm->res.i_pict_h);
                if(mctx->i_cropen)
                {
                ...
                if(mctx->i_cropen)
                {
                    mctx->i_max_w = _ALIGN_(4, parm->res.i_crop_w);
                    mctx->i_max_h = _ALIGN_(4, parm->res.i_crop_h);
                }
                else
                {
    ...
    mmfe_buff*  buff)
{
    ...
    do
    {
        if (MMFE_CTX_STATE_IDLE == mctx->i_state && MMFE_OMODE_MMAP == mctx->i_omode)
        {
            ...
            mcfg.dma.pFlushCacheCallback = out->planes[0].mem.pFlushCacheCallback;
