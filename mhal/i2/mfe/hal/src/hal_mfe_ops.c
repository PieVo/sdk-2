...
//                        pmbr->i_LutRoiQp[roiidx]);
    ...
    if(!mfe6->p_recn)
    {
        ...
        /* Free the reference picture buffer when I frame is encountered */
        if(sh->b_idr_pic)   // EROY CHECK
        {
            for(i = 0; i < mfe6->i_refn; i++)
            {
                if(NULL != (rpb = mfe6->m_dpbs[i]))
                {
                    if(!rpb->b_valid)
...
static int _EncDone(mhve_ops* mops)
{
    ...
    {
        MFE_MSG(MFE_MSG_ERR, "> MFE buffer full \n");
        return -1;
    }
...
static int _PutData(mhve_ops* mops, void* user, int size)
{
    ...
    if(size > 0)
    {
        ...
        mfe6->flush_cache_callback((void *)mfe6->p_okptr, (bcnt + 7) / 8);
...
static int _EncConf(mhve_ops* mops)
{
    ...
    {
        regs->reg0a_g_ref_y_addr0_lo = (ushort)(mfe6->m_dpbs[0]->u_phys[RPB_YPIX] >> 8);
        regs->reg0b_g_ref_y_addr0_hi = (ushort)(mfe6->m_dpbs[0]->u_phys[RPB_YPIX] >> 24);
        regs->reg0e_g_ref_c_addr0_lo = (ushort)(mfe6->m_dpbs[0]->u_phys[RPB_CPIX] >> 8);
        regs->reg0f_g_ref_c_addr0_hi = (ushort)(mfe6->m_dpbs[0]->u_phys[RPB_CPIX] >> 24);
    }
    ...
    switch(mcfg->type)
    {
        ...
        case MHVE_CFG_DMA:
            ...
            else if(mcfg->dma.i_dmem == -1)
            {
                ...
                mfe6->flush_cache_callback = mcfg->dma.pFlushCacheCallback;
        ...
        case MHVE_CFG_SPL:
            ...
            sps = &mfe6->m_sps;
            pps = &mfe6->m_pps;
            if(mfe6->i_bits || mfe6->i_rows)
            {
                pps->b_cabac_i = 1;
                pps->b_cabac_p = 1;
                if(sps->i_profile_idc < MHVE_AVC_PROFILE_MP)
                    sps->i_profile_idc = MHVE_AVC_PROFILE_MP;
            }
            err = 0;
            break;
