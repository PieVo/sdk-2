...
{
    return BsWrite(bs, (int)ue_code_lengths[val+1], val+1);
}
...
static int BsWriteSE(bs_t* bs, int val)
{
    ...
    return BsWrite(bs, size, (uint)val);
}
...
//------------------------------------------------------------------------------
static int VuiGetAspectRatioIdc(int sar_w, int sar_h)
{
    int sar_idc = 0;
...
static int VuiWriteParam(bs_t* bs, vui_t* vui)
{
    ...
    int sar_idc = VuiGetAspectRatioIdc(vui->i_sar_w, vui->i_sar_h);
    ...
    else
    {
        ...
        BsWrite(bs, 8, sar_idc /* aspect_ratio_idc */);
    ...
    BsWriteU1(bs, 0/* overscan_info_present_flag */);
...
int AvcWriteSliceHeader(bs_t* bs, slice_t* sh)
{
    ...
    if (sh->i_ref_idc)
    {
        if (sh->b_idr_pic)
        {
            BsWriteU1(bs, 0/*no_output_prior_pics_flag*/);
            BsWriteU1(bs, 0/*long_term_reference_flag*/);
        }
        else
            BsWriteU1(bs, 0/*adaptive_ref_pic_marking_mode_flag*/);
    }
