...
//LOOP_TEST:
    {
        ...
        RcCtl.stAttrH264Cbr.u32SrcFrmRate = 30;
        ...
        setHeader(&RcCtl, sizeof(RcCtl));
        MHAL_MFE_GetParam(gMhalMfeInst0, E_MHAL_VENC_264_RC, &RcCtl);
        CamOsPrintf("[KUT] RC %d %d %d\n", RcCtl.stAttrH264Cbr.u32Gop, RcCtl.stAttrH264Cbr.u32SrcFrmRate, RcCtl.stAttrH264Cbr.u32BitRate);
#endif
...
//    sprintf(szFn, "/tmp/%dx%d_0.h264", YUV_WIDTH,YUV_HEIGHT);
...
//        CamOsPrintf("Open %s failed\n", szFn);
        ...
        while(1)
        {
            ...
            for(i = 0; i < 16; i++)
            {
                ...
                {
                    md5_compare = 0;
                    printk(KERN_WARNING "MFE Core 0: MD5 compare err !!! Exit burn-in test\n");
                    break;
                }
            }
#endif
            ...
            if(md5_compare)
            {
                printk(KERN_WARNING "MFE Core 0: MD5 compare success !!\n");
            }
            else
            {
                printk(KERN_WARNING "MFE Core 0: MD5 compare fail !!\n");
                break;
            }
...
static int _CamOsThreadTestEntry1(void *pUserdata)
{
    ...
    if(0 != request_irq(MfeIrq, _MfeUtIsr_1, IRQF_SHARED, "_MfeUtIsr", gMhalMfeDev1))
    {
...
//    sprintf(szFn, "/tmp/%dx%d_1.h264", YUV_WIDTH,YUV_HEIGHT);
//    outfp = OpenFile(szFn, O_RDWR | O_CREAT | O_TRUNC, 0644);
//    if(outfp)
//        CamOsPrintf("Open %s success\n", szFn);
//    else
//        CamOsPrintf("Open %s failed\n", szFn);
        ...
        while(1)
        {
            ...
            for(i = 0; i < 16; i++)
            {
                ...
                {
                    md5_compare = 0;
                    printk(KERN_WARNING "MFE Core 1: MD5 compare err !!! Exit burn-in test\n");
                    break;
                }
            }
#endif
            ...
            if(md5_compare)
            {
                printk(KERN_WARNING "MFE Core 1: MD5 compare success !!\n");
            }
            else
            {
                printk(KERN_WARNING "MFE Core 1: MD5 compare fail !!\n");
                break;
            }
