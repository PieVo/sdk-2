...
#if !defined(HAL_K6_SIMULATE) || !defined(HAL_I3_SIMULATE)
extern bool MDrv_MIU_Protect(u8 u8Blockx, u16 *pu8ProtectId, unsigned long long u64BusStart, unsigned long long u64BusEnd, bool bSetFlag);
#endif
...
static unsigned int   gu32MiuProtectSet = 0;
...
void _MHAL_CMDQ_InitCmdqBuffer(MHAL_CMDQ_Mmap_Info_t *pCmdqMmapInfo)
{
    ...
    /*Because MI get 0 base address , then need to protect oit*/
    if(gCmdBufferHead.phyCmdqBufPhyAddr == 0x00000000 && !gu32MiuProtectSet)
    {
        u16 cpuid[]={0x70};
        int miuBlockIndex = 2;
        bool bSetFlag=TRUE;
        ...
        gu32MiuProtectSet = 1;
        MDrv_MIU_Protect(miuBlockIndex, cpuid, 0x20000000, 0x20100000, bSetFlag);

        CMDQ_LOG("[CMDQ]Set MIU protect base-size(0x%x-0x%x)\n",(MS_U32)gCmdBufferHead.phyCmdqBufPhyAddr
                  ,(MS_U32)gCmdBufferHead.u32CmdqBufSize);
    }
...
MHAL_CMDQ_CmdqInterface_t *MHAL_CMDQ_GetSysCmdqService(MHAL_CMDQ_Id_e eCmdqId, MHAL_CMDQ_BufDescript_t *pCmdqBufDesp, MS_BOOL bForceRIU)
{
    ...
    if(!gCmdqInitDone)
    {
        ...
        {
            gCmdqOps[i].MHAL_CMDQ_GetNextMlodRignBufWritePtr     = MHAL_CMDQ_GetNextMlodRignBufWritePtr;
