...
static uint32 fLog2_range1_to_2(uint32 x)
{
    ...
    if(x == 0)
    {
        ...
        assert(0); // represents negative infinity
    }
...
static uint32 fPow_frac(uint32 A, int exponment)
{
    ...
    // Overflow checking  //derek check
//  assert(abs_Y_x_abs_ln_A<=INT_MAX);
...
uint64 MhefPow64(uint32 base, int exponment)
{
    ...
    uint32 pow_frac = fPow_frac(base, fraction);
    uint64 pow_int = fPow64_int(base, integer);

    uint64 rst = ((uint64)pow_frac) * pow_int;
    rst >>= 16;
...
static uint64 fPow64_int_high_order(uint32 base, int exponment)
{
    ...
    uint64 pwr = base << POW_INT_EXTEND_BIT;
...
static uint64 fPow64_int(uint32 base, int exponment)
{
    ...
    return (exponment > 0) ? rst : CamOsMathDivS64(((uint64)1) << 32, rst, &uiremainder);
}
