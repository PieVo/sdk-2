...
unsigned int bs_read(int N)
{
    ...
    else
    {
        do
        {
            ...
            if(remindbits < 16)
            {
                Val = Show_Bits(remindbits);
                Flush_Buffer(remindbits);
                count += remindbits;
                break;
            }
            else
...
unsigned int bs_read_u1(void)
{
    return bs_read(1);
}

void Fill_Buffer(unsigned char *pData, unsigned int Datalength)
{
    int Buffer_Level;
...
#define _get_ue(coment)       bs_read_ue()
#define _get_se(coment)       bs_read_se()
#define _get_u1(coment)       bs_read_u1()
#define _get(N, coment)       bs_read(N)
...
//                {
                _put(8, sh->poc_lsb_lt[0],                  "poc_lsb_lt[0]");
...
static pps_t pps_def =
{
    ...
    .b_deblocking_filter_control_pres = 1, //derek check
    ...
    int         rpbn)
{
    ...
    rps = &sps->rps[1];
    rps->i_id = 1;
    rps->i_num_neg_pics = 0;//1;    //derek check
    rps->i_dpoc[0] = -2;
    rps->b_used[0] = 1;
    rps->i_num_pos_pics = 0;
    ...
    h265_enc*   h265)
{
    ...
    if(sps->b_conformance_window)
    {
        ...
        sps->i_crop_r = (sps->i_pic_w_min_cbs - h265->i_picw) >> 1;
        sps->i_crop_b = (sps->i_pic_h_min_cbs - h265->i_pich) >> 1;
    }
#endif
    h265->i_poc = 0;
    ...
    h265_enc* h265)
{
    ...
    //Initialize_Buffer(h265->m_hwsh+START_CODE_LEN, TotalLength-START_CODE_LEN);
...
//        pSh->i_poc =                        _get(8,                         "slice_pic_order_cnt_lsb");
    ...
    used_bits = get_used_bits() - NAL_LEN + ALIGN_EQU_ONE;
