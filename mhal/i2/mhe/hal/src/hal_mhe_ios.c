...
#define  GEN_REG_CMD(base, idx, val, buf, buf_cnt, buf_max) (*(u64*)(cmd_buf+((*cmd_len<(buf_max-1))?(*cmd_len)++:(*cmd_len))*8) = ((u64)(u32)base+4*idx) | ((u64)val & 0x0000FFFF) << 32)
...
static int _EncFire(mhve_ios* ios, mhve_job* job)
{
    ...
    if(regs->hev_bank0.reg_hev_ec_qp_delta_enable_flag)
    {
    ...
    {
        regs->hev_bank1.reg00 = 0;
        regs->hev_bank1.reg01 = 0;
        regs->hev_bank1.reg_mbr_const_qp_en = 1;
        regs->hev_bank1.reg02 = 0;
        regs->hev_bank1.reg_mbr_ss_turn_off_perceptual = 1;
        regs->hev_bank1.reg03 = 0;
        regs->hev_bank1.reg04 = 0;
        regs->hev_bank1.reg05 = 0;
        regs->hev_bank1.reg06 = 0;
        regs->hev_bank1.reg07 = 0;
        regs->hev_bank1.reg08 = 0;
        regs->hev_bank1.reg09 = 0;
        regs->hev_bank1.reg0a = 0;
        regs->hev_bank1.reg0b = 0;
        regs->hev_bank1.reg0c = 0;
...
static int _EncFireGenCmd(mhve_ios* ios, mhve_job* job, void* cmd_buf, int* cmd_len)
{
    ...
    *cmd_len = 0;
    ...
    /* Flush regs to mhe-hw */
    GEN_REG_CMD(hev_base0, 0x1c, 0x00, cmd_buf, cmd_len, cmd_len_max); //MskIrq(base, 0x00);
...
static int _IsrFunc(mhve_ios* ios, int irq)
{
    ...
    regs->enc_cycles = ((uint)(regs->mhe_bank0.reg75 & 0xFF) << 16) + regs->mhe_bank0.reg74;
    regs->enc_bitcnt = ((uint)(regs->hev_bank1.reg_ro_hev_ec_bsp_bit_cnt_high) << 16) + regs->hev_bank1.reg_ro_hev_ec_bsp_bit_cnt_low;
    //CamOsPrintf("> [%s %d] %p: bitstream size (%d %d)\n", __FUNCTION__, __LINE__, hev_base0, regs->enc_bitcnt, regs->enc_bitcnt / 8);
    //regs->enc_sumpqs = ((uint)(regs->regf6_s_mbr_last_frm_avg_qp_hi)<<16) + regs->regf5_s_mbr_last_frm_avg_qp_lo;
    regs->mjob.i_tick = (int)(regs->enc_cycles);
    regs->mjob.i_bits = (int)(regs->enc_bitcnt - regs->bits_delta); // Get the bit counts of pure bitstream and slice header.

    //CamOsPrintf("> [%s %d] bitstream size (i_bits %d, bits_delta %d)\n", __FUNCTION__, __LINE__, regs->mjob.i_bits, regs->bits_delta);

    for(i = 0; i < PMBR_LUT_SIZE; i++)
    {
        regaddr = 0x16 + i;
        regs->pmbr_lut_hist[i] = REGRD(hev_base1, regaddr, "LUT entry histogram entry");
        //CamOsPrintf("[%s %d] regs->pmbr_lut_hist[%d] = %d\n",__FUNCTION__,__LINE__, i, regs->pmbr_lut_hist[i]);
    }
