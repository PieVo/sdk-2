...
#define MHE_MSG_LEVEL    MHE_MSG_ERR //JPE_MSG_WARNING
...
#define CDBZ(v)   do if(v==0){MHE_MSG(MHE_MSG_ERR,"[%s %d] Division by zero\n",__FUNCTION__,__LINE__);}while(0)

//check assert
#define assert(v)   do if((v)==0){MHE_MSG(MHE_MSG_ERR,"[%s %d] assert !!\n",__FUNCTION__,__LINE__);}while(0)
...
//IMI Buffer
//#define MMHE_IMI_BUF_ADDR 0x001800
