    ...
    do
    {
        if(on)
        {
            ...
            do
            {
                ...
                //CamOsPrintf("[IMI]gn_mem: 0x%08X\n", mcfg.hev_buff_addr.gn_mem);
                mops->set_conf(mops, &mcfg);
                ...
                CamOsPrintf("[IMI] ppu_int_b: 0x%08X\n", mcfg.hev_buff_addr.ppu_int_b);
                mops->set_conf(mops, &mcfg);
                ...
                CamOsPrintf("[IMI] imi_ref_y_buf: 0x%08X\n", mcfg.hev_buff_addr.imi_ref_y_buf);
                mops->set_conf(mops, &mcfg);
                current_sram_addr += size_imi_ref_y;

                mcfg.type = MHVE_CFG_BUF;
                mcfg.hev_buff_addr.imi_ref_c_buf = (u32)current_sram_addr;
                CamOsPrintf("[IMI] imi_ref_c_buf: 0x%08X\n", mcfg.hev_buff_addr.imi_ref_c_buf);
                mops->set_conf(mops, &mcfg);
                current_sram_addr += size_imi_ref_c;
#endif

#if 1
                /* reconstructed/reference Y/C buffer */
                mcfg.type = MHVE_CFG_BUF;
                mcfg.hev_buff_addr.refconY[0] = (u32)nMiuAddr2;
                mops->set_conf(mops, &mcfg);
                //CamOsPrintf("mcfg.hev_buff_addr.refconY[0] = 0x%p\n",mcfg.hev_buff_addr.refconY[0]);
                nMiuAddr2 += _ALIGN_(9, size_ppu_y);

                mcfg.type = MHVE_CFG_BUF;
                mcfg.hev_buff_addr.refconC[0] = (u32)nMiuAddr2;
                mops->set_conf(mops, &mcfg);
                //CamOsPrintf("mcfg.hev_buff_addr.refconC[0] = 0x%p\n",mcfg.hev_buff_addr.refconC[0]);
                nMiuAddr2 += _ALIGN_(9, size_ppu_c);

                mcfg.type = MHVE_CFG_BUF;
                mcfg.hev_buff_addr.refconY[1] = (u32)nMiuAddr2;
                mops->set_conf(mops, &mcfg);
                //CamOsPrintf("mcfg.hev_buff_addr.refconY[1] = 0x%p\n",mcfg.hev_buff_addr.refconY[1]);
                nMiuAddr2 += _ALIGN_(9, size_ppu_y);

                mcfg.type = MHVE_CFG_BUF;
                mcfg.hev_buff_addr.refconC[1] = (u32)nMiuAddr2;
                mops->set_conf(mops, &mcfg);
                //CamOsPrintf("mcfg.hev_buff_addr.refconC[1] = 0x%p\n",mcfg.hev_buff_addr.refconC[1]);
                nMiuAddr2 += _ALIGN_(9, size_ppu_c);

                mcfg.type = MHVE_CFG_BUF;
                mcfg.hev_buff_addr.colsadr[0] = (u32)nMiuAddr2;
                mops->set_conf(mops, &mcfg);
                //CamOsPrintf("mcfg.hev_buff_addr.colsadr[0] = 0x%p\n",mcfg.hev_buff_addr.colsadr[0]);
                nMiuAddr2 += _ALIGN_(9, size_col);

                mcfg.type = MHVE_CFG_BUF;
                mcfg.hev_buff_addr.colsadr[1] = (u32)nMiuAddr2;
                mops->set_conf(mops, &mcfg);
                //CamOsPrintf("mcfg.hev_buff_addr.colsadr[1] = 0x%p\n",mcfg.hev_buff_addr.colsadr[1]);
    ...
    if(mctx->i_state == MMHE_CTX_STATE_NULL)
    {
        ...
        switch(parm->type)
        {
            case MMHE_PARM_RES:
                ...
                mctx->i_cropen = parm->res.i_cropen;
                if(mctx->i_cropen)
                {
                    ...
                    mctx->i_max_w = _ALIGN_(5, parm->res.i_crop_w);
                    mctx->i_max_h = _ALIGN_(5, parm->res.i_crop_h);
                }
                else
                {
                    mctx->i_max_w = _ALIGN_(5, parm->res.i_pict_w);
                    mctx->i_max_h = _ALIGN_(5, parm->res.i_pict_h);
                }
    ...
    mmhe_parm*      parm)
{
    ...
    switch(parm->type)
    {
        ...
        case MMHE_PARM_RES:
            ...
            parm->res.i_cropen = mctx->i_cropen;
        ...
        case MMHE_CTRL_SEQ:
            if(mctx->i_state > MMHE_CTX_STATE_NULL)
            {
                if((unsigned)ctrl->seq.i_pixfmt <= MMHE_PIXFMT_YVYU &&
                        ...
                        ctrl->seq.i_pixelw <= mctx->i_max_w &&
                        ctrl->seq.d_fps > 0 &&
                        ctrl->seq.n_fps > 0)
                {
                    int mbw, mbh, mbn;
