...
void HalSclDmaReSetHw(void)
{
    ...
    HalUtilityW2BYTEMSKDirect(REG_SCL0_06_L,0x0100, 0x0F00);
    ...
    HalUtilityW2BYTEMSKDirect(REG_SCL0_02_L, BIT8, BIT8);
...
#if CONFIG_OF
void HalSclDmaFCLKInit(bool bEn,DrvSclDmaClkConfig_t *stclk)
{
#if 0
...
void HalSclDmaFCLK2Init(bool bEn,DrvSclDmaClkConfig_t *stclk)
{
#if OSDinverseBug
#else
    DrvSclOsClkStruct_t* stclock = NULL;
...
void HalSclDmaFCLKInit(bool bEn,DrvSclDmaClkConfig_t *stclk)
{
    ...
    {
        HalUtilityW2BYTEMSK(REG_SCL_CLK_51_L,DISABLE_CLK,0x001F);//h61
#if OSDinverseBug
        HalUtilityW2BYTEMSK(REG_SCL_CLK_51_L,(DISABLE_CLK<<8),0x1F00);//h61
#endif
    }
#endif
}
...
void HalSclDmaODCLKInit(bool bEn,DrvSclDmaClkConfig_t *stclk)
{
    ...
    if(bEn)
    {
        if((HalUtilityR2BYTEDirect(REG_SCL_CLK_52_L)&DISABLE_CLK))
        {
            HalUtilityW2BYTEMSK(REG_SCL_CLK_52_L,0x000C,0x000F);//h66
        }
        else
        {
            sclprintf("[odclk]CAN'T/Already OPEN\n");
    ...
    {
        HalUtilityW2BYTEMSK(REG_SCL_CLK_52_L,0x000D,0x000F);//h66
    }
    #endif
}
...
u16 HalSclDmaGetDMAOutputCount(DrvSclDmaClientType_e enClientType)
{
    ...
    switch(enClientType)
    {
        ...
        case E_DRV_SCLDMA_4_FRM_W:
            ...
            u16def = HalUtilityR2BYTEDirect(REG_MDWIN1_2D_L);
            break;
        default:
            u16Idx = 0x0;
            u16def = 0x0;
