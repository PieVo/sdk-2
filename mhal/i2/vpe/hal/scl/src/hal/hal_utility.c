...
DrvSclOsCmdqInterface_t gpstCmdq;
...
u16 _HalUtilityWrite2ByteRegBuffer(u32 u32Reg,u16 u16Val,u16 u16Mask)
{
    ...
    {
        SCL_DBGERR("[SCLUTILITY]enType Over 2ByteReg:%lx val:%hx mask:%hx \n",u32Reg,u16Val,u16Mask);
        return 0;
    }
}
...
u16 _HalUtilityWrite2ByteInquireBuffer(u32 u32Reg,u16 u16Val,u16 u16Mask)
{
    ...
    bHByte = (u32Reg&0x1);
    bNewBufferCnt = HalUtilityGetCmdqBufferIdx(&u16BufferCnt,u32LocalReg);
    u32Bank = (u32Reg&0xFFFF00);
    enType = _HalUtilitySwitchBankToType(u32Bank);
    u32TypeMax = _HalUtilityGetMax();
    if(enType< u32TypeMax &&gpvInquireBuffer)
    {
        ...
        if(bNewBufferCnt)
        {
            ...
            HalUtilitySetCmdqBufferIdx((*gu32TblCnt),u32LocalReg);
        ...
        pvBankaddr->u16Data = _HalUtilityReadRegBuffer(u32LocalReg);
...
void HalUtilityGetMloadHandler(void **pvMloadhandler)
{
    ...
    {
        attr.mode = (gpstCmdq.MHAL_CMDQ_WriteRegCmdqMask) ? CMDQ_MODE : RIU_MODE;
        attr.pCmqInterface_t = &gpstCmdq;
...
void HalUtilitySetMloadTbl(u32 u32Id ,void *pvMloadHandler,void *pvBuf)
{
    ...
    HalUtilityW2BYTEMSKDirectCmdq(REG_SCL_MLOAD_0B_L,u16bankB,0xFFFF);//B
    HalUtilityW2BYTEMSKDirectCmdq(REG_SCL_MLOAD_00_L,u32Id-MLOAD_ID_BASE,0x1F);//0
    HalUtilityW2BYTEMSKDirectCmdq(REG_SCL_MLOAD_03_L,(u32Addr&0xFFFF),0xFFFF);// 3
    HalUtilityW2BYTEMSKDirectCmdq(REG_SCL_MLOAD_04_L,((u32Addr>>16)),0xFFFF);// 4
    HalUtilityW2BYTEMSKDirectCmdq(REG_SCL_MLOAD_01_L,u16bank1,0xFFFF);// 1
    HalUtilityW2BYTEMSKDirectCmdq(REG_SCL_MLOAD_02_L,0x0000,0xFFFF);// 2
    HalUtilityW2BYTEMSKDirectCmdq(REG_SCL_MLOAD_09_L,0x1,0x1);// 9
    HalUtilityW2BYTEMSKDirectCmdq(REG_SCL_MLOAD_09_L,0x2,0x2);// 9
...
void HalUtilityGetMloadIdle(void)
{
    u8 u8Cnt = 0;
    if((DrvSclOsGetAccessRegMode() ==  E_DRV_SCLOS_AccessReg_CMDQ)&& gpstCmdq.MHAL_CMDQ_CmdqPollRegBits)
    {
        gpstCmdq.MHAL_CMDQ_CmdqPollRegBits(&gpstCmdq,REG_SCL_MLOAD_0A_L,0x0000,0x0001,1);
    }
    else
    {
        for(u8Cnt=0;u8Cnt<100;u8Cnt++)
        {
            if((HalUtilityR2BYTEDirect(REG_SCL_MLOAD_0A_L)&0x0001)==0)
            {
                break;
            }
...
void HalUtilitySetWaitMload(void)
{
    ...
    if((DrvSclOsGetAccessRegMode() ==  E_DRV_SCLOS_AccessReg_CMDQ)&& gpstCmdq.MHAL_CMDQ_CmdqPollRegBits)
    {
        gpstCmdq.MHAL_CMDQ_CmdqPollRegBits(&gpstCmdq,REG_SCL0_17_L,0x1000,0x1000,1);
        gpstCmdq.MHAL_CMDQ_WriteRegCmdqMask(&gpstCmdq,REG_SCL0_13_L,0x1000,0x1000);
        gpstCmdq.MHAL_CMDQ_WriteRegCmdqMask(&gpstCmdq,REG_SCL0_13_L,0,0x1000);
    }
    else
    {
        for(u8Cnt=0;u8Cnt<100;u8Cnt++)
...
void HalUtilityFilpRegFire(void)
{
    u32 u32TblCnt;
    HalUtilityCmdReg_t *pvBankaddr;
    u32 i;
    if((DrvSclOsGetAccessRegMode() ==  E_DRV_SCLOS_AccessReg_CMDQ)&& gpstCmdq.MHAL_CMDQ_WriteRegCmdqMask)
    {
        //get cnt (for loop)
        ...
        for(i=0;i<u32TblCnt;i++)
        {
           ...
           //get cmd(by addr)
           //pf();
           gpstCmdq.MHAL_CMDQ_WriteRegCmdqMask(&gpstCmdq,pvBankaddr->u32Addr,pvBankaddr->u16Data,pvBankaddr->u16Mask);
...
void HalUtilityW2BYTEMSKDirectCmdq(u32 u32Reg,u16 u16Val,u16 u16Mask)
{
    ...
    {
        _HalUtilityWrite2ByteRegBuffer(u32Reg, u16Val, u16Mask);
        gpstCmdq.MHAL_CMDQ_WriteRegCmdqMask(&gpstCmdq,u32Reg,u16Val,u16Mask);
    }
    else
    {
        _HalUtilityWrite2ByteRegBuffer(u32Reg, u16Val, u16Mask);
...
void HalUtilityKeepCmdqFunction(DrvSclOsCmdqInterface_t *pstCmdq)
{
    if(pstCmdq)
    {
        DrvSclOsMemcpy(&gpstCmdq,pstCmdq,sizeof(DrvSclOsCmdqInterface_t));
        //ToDo
    }
    else
    {
        DrvSclOsMemset(&gpstCmdq,0,sizeof(DrvSclOsCmdqInterface_t));
    }
}
void * HalUtilityReturnCmdqWriteFunction(void)
{
    return (void *)gpstCmdq.MHAL_CMDQ_WriteRegCmdqMask;
}
bool HalUtilityDeInit(void)
{
    ...
    if(_UTILITY_Mutex != -1)
    {
        DrvSclOsDeleteMutex(_UTILITY_Mutex);
        _UTILITY_Mutex = -1;
    }
    return TRUE;
}
