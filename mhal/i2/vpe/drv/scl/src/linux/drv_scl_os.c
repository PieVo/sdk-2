...
static                          DEFINE_SPINLOCK(_SclOs_Clk_Mutex);
#define CLK_MUTEX_LOCK()      spin_lock(&_SclOs_Clk_Mutex)
#define CLK_MUTEX_UNLOCK()    spin_unlock(&_SclOs_Clk_Mutex)
...
DrvSclOsVipSetRuleType_e genVIPSetRule = E_DRV_SCLOS_VIPSETRUle_DEFAULT;
...
{
    poll_wait((struct file *)stPollWait->filp, (wait_queue_head_t *)stPollWait->pWaitQueueHead,
        (struct poll_table_struct *)stPollWait->pstPollQueue);
    return 1;
}
...
#if I2_DVR_TEST
void DrvSclOsTestVpeIq(void)
{
    u32 u32Ctx = 0;
    int i;
    int a;
    MHalVpeIqConfig_t stIqCfg;
    MHalVpeIqOnOff_t stIqOnCfg;
    MHalVpeIqWdrRoiReport_t stRoiReport;
    MHalVpeIqWdrRoiHist_t stHistCfg;
    DrvSclOsMemset(&stIqCfg,0,sizeof(MHalVpeIqConfig_t));
    DrvSclOsMemset(&stIqOnCfg,0,sizeof(MHalVpeIqOnOff_t));
    DrvSclOsMemset(&stRoiReport,0,sizeof(MHalVpeIqWdrRoiReport_t));
    DrvSclOsMemset(&stHistCfg,0,sizeof(MHalVpeIqWdrRoiHist_t));
    stHistCfg.bEn = 0;
    stHistCfg.enPipeSrc = E_MHAL_IQ_ROISRC_BEFORE_WDR;
    stHistCfg.u8WinCount = 4;
    for(i=0;i<4;i++)
    {
        stHistCfg.stRoiCfg[i].bEnSkip = 0;
        stHistCfg.stRoiCfg[i].u16RoiAccX[0] = 100+i*100;
        stHistCfg.stRoiCfg[i].u16RoiAccY[0] = 100+i*100;
        stHistCfg.stRoiCfg[i].u16RoiAccX[1] = 200+i*100;
        stHistCfg.stRoiCfg[i].u16RoiAccY[1] = 100+i*100;
        stHistCfg.stRoiCfg[i].u16RoiAccX[2] = 200+i*100;
        stHistCfg.stRoiCfg[i].u16RoiAccY[2] = 200+i*100;
        stHistCfg.stRoiCfg[i].u16RoiAccX[3] = 100+i*100;
        stHistCfg.stRoiCfg[i].u16RoiAccY[3] = 200+i*100;
    }
    MHalVpeCreateIqInstance(NULL ,(void *)&u32Ctx);
    MHalVpeIqDbgLevel((void *)&a);
    MHalVpeIqProcess((void *)u32Ctx, NULL);
    MHalVpeIqConfig((void *)u32Ctx, &stIqCfg);
    MHalVpeIqOnOff((void *)u32Ctx, &stIqOnCfg);
    MHalVpeIqSetWdrRoiHist((void *)u32Ctx, &stHistCfg);
    MHalVpeIqGetWdrRoiHist((void *)u32Ctx, &stRoiReport);
    SCL_ERR("[VPE]stRoiReport %u %u %u %u\n",
        stRoiReport.u32Y[0],stRoiReport.u32Y[1],stRoiReport.u32Y[2],stRoiReport.u32Y[3]);
    MHalVpeIqSetWdrRoiMask((void *)u32Ctx,0, NULL);
    MHalVpeIqSetDnrTblMask((void *)u32Ctx,0, NULL);
    MHalVpeIqRead3DNRTbl((void *)u32Ctx);
    MHalVpeDestroyIqInstance((void *)u32Ctx);
}
void DrvSclOsTestVpe(void)
{
    MHalVpeSclWinSize_t stMaxWin;
    MHalVpeSclCropConfig_t stCropCfg;
    MHalVpeSclInputSizeConfig_t stInputpCfg;
    MHalVpeSclOutputSizeConfig_t stOutputCfg;
    MHalVpeSclOutputDmaConfig_t stOutputDmaCfg;
    MHalVpeSclOutputMDwinConfig_t stMDwinCfg;
    MHalVpeSclOutputBufferConfig_t stpBuffer;
    u32 u32Ctx = 0;
    u32 IrqNum = 0;
    u32 a = 1;
    DrvSclOsMemset(&stMaxWin,0,sizeof(MHalVpeSclWinSize_t));
    DrvSclOsMemset(&stCropCfg,0,sizeof(MHalVpeSclCropConfig_t));
    DrvSclOsMemset(&stInputpCfg,0,sizeof(MHalVpeSclInputSizeConfig_t));
    DrvSclOsMemset(&stOutputCfg,0,sizeof(MHalVpeSclOutputSizeConfig_t));
    DrvSclOsMemset(&stOutputDmaCfg,0,sizeof(MHalVpeSclOutputDmaConfig_t));
    DrvSclOsMemset(&stMDwinCfg,0,sizeof(MHalVpeSclOutputMDwinConfig_t));
    DrvSclOsMemset(&stpBuffer,0,sizeof(MHalVpeSclOutputBufferConfig_t));
    stMaxWin.u16Height = 1080;
    stMaxWin.u16Width = 1920;
    stCropCfg.bCropEn = 0;
    stCropCfg.stCropWin.u16Height = 1080;
    stCropCfg.stCropWin.u16Width = 1920;
    stCropCfg.stCropWin.u16X = 0;
    stCropCfg.stCropWin.u16Y = 0;
    stInputpCfg.eCompressMode = E_MHAL_COMPRESS_MODE_NONE;
    stInputpCfg.ePixelFormat = E_MHAL_PIXEL_FRAME_YUV422_YUYV;
    stInputpCfg.u16Height = 1080;
    stInputpCfg.u16Width = 1920;
    stOutputCfg.enOutPort = E_MHAL_SCL_OUTPUT_PORT0;
    stOutputCfg.u16Height = 1080;
    stOutputCfg.u16Width = 1920;
    stOutputDmaCfg.enCompress = E_MHAL_COMPRESS_MODE_NONE;
    stOutputDmaCfg.enOutFormat = E_MHAL_PIXEL_FRAME_YUV422_YUYV;
    stOutputDmaCfg.enOutPort = E_MHAL_SCL_OUTPUT_PORT0;
    stMDwinCfg.enOutPort = E_MHAL_SCL_OUTPUT_PORT3;
    stMDwinCfg.enOutType = E_MHAL_SCL_OUTPUT_MDWIN;
    stpBuffer.stCfg[E_MHAL_SCL_OUTPUT_PORT0].bEn = 0;
    stpBuffer.stCfg[E_MHAL_SCL_OUTPUT_PORT1].bEn = 0;
    stpBuffer.stCfg[E_MHAL_SCL_OUTPUT_PORT2].bEn = 0;
    stpBuffer.stCfg[E_MHAL_SCL_OUTPUT_PORT3].bEn = 0;
    stpBuffer.stCfg[E_MHAL_SCL_OUTPUT_PORT0].stBufferInfo.u32Stride[0] = 0;
    stpBuffer.stCfg[E_MHAL_SCL_OUTPUT_PORT0].stBufferInfo.u64PhyAddr[0] = 0x4000000;
    stpBuffer.stCfg[E_MHAL_SCL_OUTPUT_PORT1].stBufferInfo.u32Stride[0] = 0x100;
    stpBuffer.stCfg[E_MHAL_SCL_OUTPUT_PORT1].stBufferInfo.u64PhyAddr[0] = 0x4800000;
    stpBuffer.stCfg[E_MHAL_SCL_OUTPUT_PORT2].stBufferInfo.u32Stride[0] = 0x200;
    stpBuffer.stCfg[E_MHAL_SCL_OUTPUT_PORT2].stBufferInfo.u64PhyAddr[0] = 0x4900000;
    stpBuffer.stCfg[E_MHAL_SCL_OUTPUT_PORT3].stBufferInfo.u32Stride[0] = 0x3000;
    stpBuffer.stCfg[E_MHAL_SCL_OUTPUT_PORT3].stBufferInfo.u64PhyAddr[0] = 0x4500000;

    MHalVpeCreateSclInstance(NULL, &stMaxWin, (void *)&u32Ctx);
    MHalVpeSclDbgLevel((void *)&a);
    MHalVpeSclCropConfig((void *)u32Ctx, &stCropCfg);
    MHalVpeSclInputConfig((void *)u32Ctx, &stInputpCfg);
    MHalVpeSclOutputSizeConfig((void *)u32Ctx, &stOutputCfg);
    MHalVpeSclOutputDmaConfig((void *)u32Ctx, &stOutputDmaCfg);
    MHalVpeSclOutputMDWinConfig((void *)u32Ctx, &stMDwinCfg);
    MHalVpeSclProcess((void *)u32Ctx,NULL, &stpBuffer);
    MHalVpeSclSetSwTriggerIrq(NULL);
    MHalVpeSclSetWaitDone((void *)u32Ctx,NULL,0);
    MHalVpeSclEnableIrq(1);
    MHalVpeSclGetIrqNum((unsigned int *)&IrqNum);
    MHalVpeSclClearIrq();
    MHalVpeSclCheckIrq();
    MHalVpeDestroySclInstance((void *)u32Ctx);
}
#endif
...
bool DrvSclOsObtainMutexIrq(s32 s32MutexId)
{
    ...
    spin_lock_irq(&(_SclOs_Spinlock_Info[s32MutexId].stMutex));
    return TRUE;
}
...
bool DrvSclOsReleaseMutexIrq (s32 s32MutexId)
{
    ...
    if(spin_is_locked(&(_SclOs_Spinlock_Info[s32MutexId].stMutex)))
    {
        spin_unlock_irq(&(_SclOs_Spinlock_Info[s32MutexId].stMutex));
    }
    return TRUE;
    ...
    for(s32Id=0;s32Id<SCLOS_SPINLOCK_MAX;s32Id++)
    {
        if(TRUE == _SclOs_Spinlock_Info[s32Id].bUsed)
        {
            ...
            if(spin_is_locked(&(_SclOs_Spinlock_Info[s32Id].stMutex)))
            {
                ...
                spin_unlock_irq(&(_SclOs_Spinlock_Info[s32Id].stMutex));
            }
        }
    }
    return bRet;
...
int DrvSclOsSetCmdqIrqIDFormSys(MSOS_ST_PLATFORMDEVICE *pdev,u8 u8idx,E_DRV_SCLOS_CMDQIRQ_TYPE enType)
{
...
bool DrvSclOsInit (void)
{
    ...
    // Clk
    spin_lock_init(&_SclOs_Clk_Mutex);
    for (u32I=0; u32I<SCLOS_CLK_MAX; u32I++)
    {
        _SclOs_Clk_Info[u32I].bUsed = FALSE;
        _SclOs_Clk_Info[u32I].stClk.ptIdclk = NULL;
        _SclOs_Clk_Info[u32I].stClk.ptFclk1 = NULL;
        _SclOs_Clk_Info[u32I].stClk.ptFclk2 = NULL;
        _SclOs_Clk_Info[u32I].stClk.ptOdclk = NULL;
    }
    gbInit = TRUE;
    return TRUE;
}
...
bool DrvSclOsDeleteEventGroup (s32 s32EventGroupId)
{
    ...
    init_waitqueue_head(&_SclOs_EventGroup_Info[s32EventGroupId].stSemaphore);
...
void * DrvSclOsClkGetClk(void * np,u32 idx)
{
    return of_clk_get((DrvSclDeviceNode_t *)np,idx);
}

u32 DrvSclOsClkGetEnableCount(DrvSclOsClkStruct_t * clk)
{
    u32 u32Cnt;
    u32Cnt = ((struct clk *)clk)->enable_count;
    return u32Cnt;
}
...
{
    return (u32)clk_set_parent(clk, parent);
}
u32 DrvSclOsClkPrepareEnable(DrvSclOsClkStruct_t *clk)
{
    return (u32)clk_prepare_enable(clk);
}
...
u8 DrvSclOsGetSclFrameDelay(void)
{
    ...
    pvCtx = (void *)MDrvSclCtxGetConfigCtx(MDRV_SCL_CTX_ID_DEFAULT);
    _DrvSclOsSetGlobal((MDrvSclCtxCmdqConfig_t *)pvCtx);
    return gstGlobalSclOsSet->gu8SclFrameDelay;
}
void DrvSclOsSetSclFrameDelay(u8 u8delay)
{
    ...
    pvCtx = (void *)MDrvSclCtxGetConfigCtx(MDRV_SCL_CTX_ID_DEFAULT);
    _DrvSclOsSetGlobal((MDrvSclCtxCmdqConfig_t *)pvCtx);
    gstGlobalSclOsSet->gu8SclFrameDelay = u8delay;
}
...
s32 DrvSclOsGetDevIdFromM2MId(s32 s32Handler,DrvSclosDevType_e enDevId)
{
    MDrvSclCtxConfig_t *pCtxCfg = NULL;
    s32 s32NewHandler;
    pCtxCfg = MDrvSclCtxGetInstByHandler(s32Handler,E_MDRV_SCL_CTX_ID_SC_ALL);
    s32NewHandler = pCtxCfg->s32Id[enDevId];
    return s32NewHandler;
}
