...
struct vif_dev_data{
    ...
    unsigned int isp_dma_status;
};

struct vif_dev_data vif_data; // ToDo:use kmalloc

void isp_dma_do_work(struct work_struct *work)
{
    struct vif_dev_data *data = container_of(work, struct vif_dev_data, isp_dma_wq);
    //pr_info("isp_dma_do_work %x\n", (unsigned int)data->isp_dma_status);

    DrvVif_DoDmagMCb(data->isp_dma_status);
}
...
{
	int ret = E_HAL_VIF_SUCCESS, rc = 0, i=0;
	struct resource *res;
...
static irqreturn_t isp_dma_isr(int irq, void* priv)
{
    ...
    data->isp_dma_status = HalDma_DoneIrqFinalStatus(0);

    HalDma_DoneIrqMask(0, 1);
    HalDma_DoneIrqClr(0, 1);
    HalDma_DoneIrqUnMask(0, 1);

    schedule_work(&vif_data.isp_dma_wq);
...
static int mstar_vif_probe(struct platform_device *pdev)
{
    ...
    g_CLKGEN2 = ioremap(0x1F000000|(0x100A<<9),0x200);
    pr_info("g_CLKGEN2: %p\n", g_CLKGEN2);
...
#define _BANK_TO_PHY(bank) (0x1F000000+(bank<<9))
    g_DMA_IRQ = ioremap(_BANK_TO_PHY(0x153B),0x200);
    pr_info("g_DMA_IRQ: %p\n", g_DMA_IRQ);
#undef _BANK_TO_PHY
    ...
    if (0 == request_irq(irq_isp_dma, isp_dma_isr, IRQF_TRIGGER_NONE, "isp dma interrupt", &vif_data))
        pr_info("[VIF] isp dma interrupt registered #%d\n", irq_isp_dma);
    else
        pr_err("[VIF] failed to request isp dma #%d\n",irq_isp_dma);

    INIT_WORK(&vif_data.isp_dma_wq, isp_dma_do_work);
	...
	HalDma_Init();
#if 0
    DrvVif_EnableInterrupt(0, 1); //Testing
    DrvVif_IntStatusCbReg(0, VIF_INTERRUPT_PAD2VIF_VSYNC_FALLING_EDGE, rig_cb);
#endif
#if 0
    DrvVif_SetDmagMLineCntInt(0,1); //Tsting
    DrvVif_SetDmagMLineCnt(0, 0x400);
    {
        extern int DrvVif_RegDmagMCb(void (*cb)(void));
        DrvVif_RegDmagMCb(my_cb);
    }
#endif
	return 0;
}
