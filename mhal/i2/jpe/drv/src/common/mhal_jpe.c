...
MS_S32 MHAL_JPE_EncodeOneFrame(MHAL_VENC_INST_HANDLE hInst, MHAL_VENC_InOutBuf_t* pstInOutBuf)
{
    ...
    if (mctx && pstInOutBuf)
    {
        ...
        {
            tJpeCfg.InBuf[JPE_COLOR_PLAN_LUMA].nAddr = (unsigned long)pstInOutBuf->phyInputYUVBuf1;
            tJpeCfg.InBuf[JPE_COLOR_PLAN_LUMA].nSize = pstInOutBuf->u32InputYUVBuf1Size;
        ...
        else if(JPE_RAW_NV12 == tJpeCfg.eRawFormat || JPE_RAW_NV21 == tJpeCfg.eRawFormat)
        {
            tJpeCfg.InBuf[JPE_COLOR_PLAN_LUMA].nAddr = (unsigned long)pstInOutBuf->phyInputYUVBuf1;
            tJpeCfg.InBuf[JPE_COLOR_PLAN_LUMA].nSize = pstInOutBuf->u32InputYUVBuf1Size;
            tJpeCfg.InBuf[JPE_COLOR_PLAN_CHROMA].nAddr = (unsigned long)pstInOutBuf->phyInputYUVBuf2;
            tJpeCfg.InBuf[JPE_COLOR_PLAN_CHROMA].nSize = pstInOutBuf->u32InputYUVBuf2Size;
        }

        mctx->nHeaderLen = drv_jpegenc_encoder_HwJpegMarkerInit((void *)(unsigned long)pstInOutBuf->virtOutputBuf, pstInOutBuf->u32OutputBufSize, &tJpeCfg);

        tJpeCfg.OutBuf.nAddr = (unsigned long)pstInOutBuf->phyOutputBuf + mctx->nHeaderLen;
        tJpeCfg.OutBuf.nSize = pstInOutBuf->u32OutputBufSize - mctx->nHeaderLen;
...
MS_S32 MHAL_JPE_EncodeCancel(MHAL_VENC_DEV_HANDLE hDev)
{
    JpeDev_t *mdev = (JpeDev_t *)hDev;
