...
EXTRA_CFLAGS:= -g -Werror -Wall -Wno-unused-result \
			-DMSTAR_MODULE_VERSION="MStar Module version: project_commit.sdk_commit.build_time $(shell git log -n 1 --format="%h").$(shell cd $(PROJ_ROOT)/../sdk; git log -n 1 --format="%h";)$(shell cd -;).$(shell date +%Y%m%d%H%M%S)" \
