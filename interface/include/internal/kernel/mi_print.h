...
#define DBG_ENTER()                ({do{if(MI_DEVICE_GetDebugLevel()>=MI_DBG_ALL){MI_SYS_LOG_IMPL_PrintLog(ASCII_COLOR_BLUE">>>%s[%d] \n" ASCII_COLOR_END,__FUNCTION__,__LINE__);}}while(0);})
#define DBG_EXIT_OK()              ({do{if(MI_DEVICE_GetDebugLevel()>=MI_DBG_ALL){MI_SYS_LOG_IMPL_PrintLog(ASCII_COLOR_BLUE"<<<%s[%d] \n" ASCII_COLOR_END,__FUNCTION__,__LINE__);}}while(0);})
