...
#define DBG_ENTER()                 ({do{if(_MI_PRINT_GetDebugLevel(MACRO_TO_STRING(EXTRA_MODULE_NAME))>=MI_DBG_ALL){printf(ASCII_COLOR_BLUE">>>%s[%d] \n",__FUNCTION__,__LINE__);printf(ASCII_COLOR_END);}}while(0);})
#define DBG_EXIT_OK()               ({do{if(_MI_PRINT_GetDebugLevel(MACRO_TO_STRING(EXTRA_MODULE_NAME))>=MI_DBG_ALL){printf(ASCII_COLOR_BLUE"<<<%s[%d] \n",__FUNCTION__,__LINE__);printf(ASCII_COLOR_END);}}while(0);})
