...
MI_S32 MI_VENC_ReleaseStream(MI_VENC_CHN VeChn, MI_VENC_Stream_t *pstStream)
{
    ...
    if(0)
    {
        MI_VENC_ReleaseStream_t stReleaseStream;
        memset(&stReleaseStream, 0, sizeof(stReleaseStream));
        s32Ret = MI_SYSCALL(MI_VENC_RELEASE_STREAM, &stReleaseStream);
    }
...
{
    MI_S32 s32Ret = MI_ERR_VENC_UNDEFINED;
    //MI_VENC_SetMaxStreamCnt_t stSetMaxStreamCnt;
    ...
    if(s32Ret == MI_SUCCESS)
    {
        ...
        s32Ret = MI_SYS_SetChnOutputPortDepth(&stSysChnPort, 5, u32MaxStrmCnt);
    }
    if(s32Ret == MI_SUCCESS)
    {
