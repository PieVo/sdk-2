...
MI_S32 MI_SYS_MMA_Alloc(MI_U8 *szMMAHeapName, MI_U32  blkSize ,MI_PHY *pu64PhyAddr)
{
    ...
    stMmaAlloc.szMMAHeapName[MI_SYS_MMA_HEAP_NAME_MAX_LEN-1] = 0;
...
MI_S32 MI_SYS_FlushCache(void *pVirtualAddress, MI_U32 u32Length)
{
    ...
    stFlushCache.pVirtualAddress = pVirtualAddress;
    stFlushCache.length= u32Length;
    s32Ret =  MI_SYSCALL(MI_SYS_FLUSHCACHE,&stFlushCache);
    return s32Ret;
}
...
EXPORT_SYMBOL(MI_SYS_FlushCache);
