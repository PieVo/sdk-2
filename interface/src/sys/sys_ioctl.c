...
static MI_S32 MI_SYS_IOCTL_ChnInputPortGetBuf(MI_DEVICE_Context_t *env, void *ptr)
{
    ...
    AUTO_LOCK(env, mutex);
    s32Ret = MI_SYS_IMPL_ChnInputPortGetBuf(pstChnPort ,pstBufConf , &stBufInfo , &BufHandle , arg->s32TimeOutMs);
...
static MI_S32 MI_SYS_IOCTL_ChnOutputPortGetBuf(MI_DEVICE_Context_t *env, void *ptr)
{
    ...
    MI_SYS_BUF_HANDLE BufHandle;
    AUTO_LOCK(env, mutex);
    s32Ret = MI_SYS_IMPL_ChnOutputPortGetBuf(pstChnPort , &stBufInfo , &BufHandle);
    memcpy(&(arg->stBufInfo) , &stBufInfo , sizeof(MI_SYS_BufInfo_t));
    arg->BufHandle = BufHandle;
    return s32Ret;
}
...
static MI_S32 MI_SYS_IOCTL_FlushCache(MI_DEVICE_Context_t *env, void *ptr)
{
    MI_S32 s32Ret = E_MI_ERR_FAILED;

    MI_SYS_FlushCache_t *stFlushCache;
    stFlushCache = (MI_SYS_FlushCache_t *)ptr;

    AUTO_LOCK(env, mutex);

    s32Ret = MI_SYS_IMPL_FlushCache(stFlushCache->pVirtualAddress,stFlushCache->length);
    return s32Ret;
}
...
static MI_DEVICE_WrapperFunction_t ioctl_table[E_MI_SYS_CMD_MAX] = {
    ...
    [E_MI_SYS_FLUSHCACHE] = MI_SYS_IOCTL_FlushCache,
